...
public class MessageReference implements Parcelable {
    ...
    public MessageReference(final String identity) throws MessagingException {
        ...
        if (identity.charAt(0) == IDENTITY_VERSION_1.charAt(0)) {
            ...
            if (tokens.countTokens() >= 3) {
                ...
                if (MailChat.DEBUG)
                    Log.d(MailChat.LOG_TAG, "Thawed " + toString());
            } else {
    ...
    /**
     * Serialize this MessageReference for storing in a MailChat identity.  This is a colon-delimited base64 string.
     *
            ...
            if (account != null) {
                ...
                if (folder != null) {
                    ...
                    if (message != null) {
                        ...
                        Log.d(MailChat.LOG_TAG, "Could not restore message, uid " + uid + " is unknown.");
                    }
                } else {
                    Log.d(MailChat.LOG_TAG, "Could not restore message, folder " + folderName + " is unknown.");
                }
            } else {
                Log.d(MailChat.LOG_TAG, "Could not restore message, account " + accountUuid + " is unknown.");
            }
        } catch (MessagingException e) {
            Log.w(MailChat.LOG_TAG, "Could not retrieve message for reference.", e);
        }
