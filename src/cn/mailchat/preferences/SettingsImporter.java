package cn.mailchat.preferences;
...
import cn.mailchat.preferences.Settings.InvalidSettingValueException;
...
public class SettingsImporter {
        ...
        try
        {
            ...
            if (globalSettings) {
                try {
                    ...
                    if (imported.globalSettings != null) {
                        ...
                        Log.w(MailChat.LOG_TAG, "Was asked to import global settings but none found.");
                    }
                    if (editor.commit()) {
                        if (MailChat.DEBUG) {
                            Log.v(MailChat.LOG_TAG, "Committed global settings to the preference " +
                                    "storage.");
                        }
                        globalSettingsImported = true;
                    } else {
                        if (MailChat.DEBUG) {
                            Log.v(MailChat.LOG_TAG, "Failed to commit global settings to the " +
                                    "preference storage");
                        }
                    }
                } catch (Exception e) {
                    Log.e(MailChat.LOG_TAG, "Exception while importing global settings", e);
                }
            }
            ...
            if (accountUuids != null && accountUuids.size() > 0) {
                if (imported.accounts != null) {
                    for (String accountUuid : accountUuids) {
                        if (imported.accounts.containsKey(accountUuid)) {
                            ...
                            try {
                                ...
                                if (editor.commit()) {
                                    if (MailChat.DEBUG) {
                                        Log.v(MailChat.LOG_TAG, "Committed settings for account \"" +
                                    ...
                                    if (MailChat.DEBUG) {
                                        Log.w(MailChat.LOG_TAG, "Error while committing settings for " +
                                ...
                                if (MailChat.DEBUG) {
                                    Log.e(MailChat.LOG_TAG, "Encountered invalid setting while " +
                                            "importing account \"" + account.name + "\"", e);
                                }
                                errorneousAccounts.add(new AccountDescription(account.name, account.uuid));
                            } catch (Exception e) {
                                Log.e(MailChat.LOG_TAG, "Exception while importing account \"" +
                                        account.name + "\"", e);
                                errorneousAccounts.add(new AccountDescription(account.name, account.uuid));
                            }
                        } else {
                            Log.w(MailChat.LOG_TAG, "Was asked to import account with UUID " +
                    ...
                    Log.w(MailChat.LOG_TAG, "Was asked to import at least one account but none found.");
                }
            }

            preferences.loadAccounts();
            MailChat.loadPrefs(preferences);
            MailChat.setServicesEnabled(context);
    ...
    private static void putString(SharedPreferences.Editor editor, String key, String value) {
        if (MailChat.DEBUG) {
            ...
            if (!MailChat.DEBUG_SENSITIVE &&
                    (key.endsWith(".transportUri") || key.endsWith(".storeUri"))) {
                outputValue = "*sensitive*";
            }
            Log.v(MailChat.LOG_TAG, "Setting " + key + "=" + outputValue);
        }
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if(eventType == XmlPullParser.START_TAG) {
                    if (SettingsExporter.ROOT_ELEMENT.equals(xpp.getName())) {
                        ...
                        Log.w(MailChat.LOG_TAG, "Unexpected start tag: " + xpp.getName());
                    }
                }
                eventType = xpp.next();
            }
                ...
                if (SettingsExporter.GLOBAL_ELEMENT.equals(element)) {
                    if (overview || globalSettings) {
                        if (result.globalSettings == null) {
                            ...
                            skipToEndTag(xpp, SettingsExporter.GLOBAL_ELEMENT);
                            Log.w(MailChat.LOG_TAG, "More than one global settings element. Only using the first one!");
                        }
                    } else {
                        skipToEndTag(xpp, SettingsExporter.GLOBAL_ELEMENT);
                        Log.i(MailChat.LOG_TAG, "Skipping global settings");
                    }
                } else if (SettingsExporter.ACCOUNTS_ELEMENT.equals(element)) {
                    if (result.accounts == null) {
                        result.accounts = parseAccounts(xpp, accountUuids, overview);
                    } else {
                        Log.w(MailChat.LOG_TAG, "More than one accounts element. Only using the first one!");
                    }
                } else {
                    Log.w(MailChat.LOG_TAG, "Unexpected start tag: " + xpp.getName());
                }
            ...
            if(eventType == XmlPullParser.START_TAG) {
                ...
                if (SettingsExporter.VALUE_ELEMENT.equals(element)) {
                    ...
                    if (result.settings.containsKey(key)) {
                        Log.w(MailChat.LOG_TAG, "Already read key \"" + key + "\". Ignoring value \"" + value + "\"");
                    } else {
            ...
            if(eventType == XmlPullParser.START_TAG) {
                ...
                if (SettingsExporter.ACCOUNT_ELEMENT.equals(element)) {
                    ...
                    if (account == null) {
                        ...
                        Log.w(MailChat.LOG_TAG, "Duplicate account entries with UUID " + account.uuid +
                                ". Ignoring!");
                    }
                } else {
                    Log.w(MailChat.LOG_TAG, "Unexpected start tag: " + xpp.getName());
    ...
    throws XmlPullParserException, IOException {
        ...
        try {
            ...
            Log.w(MailChat.LOG_TAG, "Skipping account with invalid UUID " + uuid);
            return null;
        }
        ...
        if (overview || accountUuids.contains(uuid)) {
            ...
            skipToEndTag(xpp, SettingsExporter.ACCOUNT_ELEMENT);
            Log.i(MailChat.LOG_TAG, "Skipping account with UUID " + uuid);
        }
