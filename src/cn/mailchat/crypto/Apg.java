...
public class Apg extends CryptoProvider {
    ...
    public boolean onActivityResult(Activity activity, int requestCode, int resultCode,
        ...
        case Apg.SELECT_SECRET_KEY:
            ...
            ((MailComposeActivity) activity).updateEncryptLayout();
            break;

        case Apg.SELECT_PUBLIC_KEYS:
            if (resultCode != Activity.RESULT_OK || data == null) {
                pgpData.setEncryptionKeys(null);
                ((MailComposeActivity) activity).onEncryptionKeySelectionDone();
                break;
            }
        ...
        case Apg.ENCRYPT_MESSAGE:
            if (resultCode != Activity.RESULT_OK || data == null) {
                pgpData.setEncryptionKeys(null);
                ((MailComposeActivity) activity).onEncryptDone();
                break;
            }
    ...
    public boolean test(Context context) {
        ...
        try {
            ...
            // if there was a problem, then let the user know, this will not stop MailChat/APG from
