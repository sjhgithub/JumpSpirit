...
public class Address {
    ...
    String mAddress;
    String mPersonal;
    String mUserHeadPath;

	private boolean isChooseListUser=false;
        	...
        	addressList="";
          	for (int i = 0; i < addressArray.length; i++) {
        		if(i==addressArray.length-1){
        			addressList+=splitName(addressArray[i]);;
        		}else{
        			addressList+=splitName(addressArray[i])+",";
        		}
			}
        }else{
        	addressList =splitName(addressList);
        }
        // END
        ...
        try {
            ...
            for (int i = 0, count = parsedList.size(); i < count; i++) {
                ...
                if (address instanceof Mailbox) {
                    ...
                    Log.e(MailChat.LOG_TAG, "Unknown address type from Mime4J: "
            ...
            if (tokens != null && tokens.length > 0) {
                for (Rfc822Token token : tokens) {
                    String mailAddress = token.getAddress();
                    String mailName = token.getName();
                    addresses.add(new Address(mailAddress, mailName, false));
                }
            } else {
                // This should be an error
                Log.e(MailChat.LOG_TAG, "MimeException in Address.parse()", pe);
    ...
    public CharSequence toFriendly(final Contacts contacts) {
        if (!MailChat.showCorrespondentNames()) {
            return mAddress;
            ...
            if (name != null) {
                if (MailChat.changeContactNameColor()) {
                    ...
                    coloredName.setSpan(new ForegroundColorSpan(MailChat.getContactNameColor()),
                                        0,
                                        coloredName.length(),
	...
	/**
	 * 合并多个Q编码的字符串便于解析,防止乱码.
	 * 如：=?utf-8?q?sdadada?= =?utf-8?q?adadad?=
	 * 注：如果出现混合编码的话这种太变态，应该不会出现,也没必要处理。
	 * 由于Q的编码是=b=a=c代表一个中文字符,所以无法使用Decoder里面的方法解码,
	 * 解出来还是乱码.所以必须要将其合并在一起解码.
	 * @Description:
	 * @param 
	 * @see:
	 * @since:
	 ...
	 * @date:2015-3-18
	 */
    public static String combinedQCharacter(String addressList){
    	 int previousEnd = 0;
         StringBuilder sb = new StringBuilder();
         String mimeCharset = null;
         String encoding = null;
         while (true) {
             int begin = addressList.indexOf("=?", previousEnd);
             if (begin == -1) {
                 sb.append(addressList.substring(previousEnd));
                 return  "=?"+mimeCharset+"?"+encoding+"?"+sb.toString()+"?=";
             }
             int qm1 = addressList.indexOf('?', begin + 2);
             if (qm1 == -1) {
                 sb.append(addressList.substring(previousEnd));
                 return sb.toString();
             }

             int qm2 = addressList.indexOf('?', qm1 + 1);
             if (qm2 == -1) {
                 sb.append(addressList.substring(previousEnd));
                 return sb.toString();
             }

             int end = addressList.indexOf("?=", qm2 + 1);
             if (end == -1) {
                 sb.append(addressList.substring(previousEnd));
                 return sb.toString();
             }
             ...
             mimeCharset = addressList.substring(begin + 2, qm1);
             encoding = addressList.substring(qm1 + 1, qm2);
             String encodedText = addressList.substring(qm2 + 1, end - 2);
	 ...
	 * @date:2015-3-18
	 */
    private static String splitName(String address){
    	if (address.contains("<")) {
			int start = address.indexOf("<");
			String newAddrName = address.substring(0, start);
			String newEmail = address.substring(start+1,
					address.length() - 1);
			if (newAddrName.equals(newEmail)) {
				address=null;
				if (newAddrName.contains("@")) {
					newAddrName=newAddrName.split("@")[0];
				}
				address=newAddrName+" <"+newEmail+">";
			}else if(StringUtil.getStringCount("=?", newAddrName)>=2){
	    		newAddrName  =combinedQCharacter(newAddrName);
	    		address=newAddrName+" <"+newEmail+">";
	    	}
		}
		return address;
	...
	public void setChooseListUser(boolean b) {
		this.isChooseListUser=b;
	}

	public boolean isChooseListUser() {
		return isChooseListUser;
	}
