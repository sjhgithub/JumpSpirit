...
//        HEADER_ANDROID_ATTACHMENT_ID,
    ...
    // Modified by LL
    // BEGIN
    public static boolean isEqual(String[] headerX, String[] headerY) {
    	if (headerX == null || headerY == null) {
    		return false;
    	}
    	
    	if (headerX.length != headerY.length) {
    		return false;
    	}
    	
    	for (int i = 0; i < headerX.length; i++) {
    		if (headerX[i] == null || headerY[i] == null) {
    			return false;
    		}
    		
    		if (!headerX[i].equals(headerY[i])) {
    			return false;
    		}
    	}
