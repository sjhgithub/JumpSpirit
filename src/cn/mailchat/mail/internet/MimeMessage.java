...
public class MimeMessage extends Message {
	...
	// 附件数
		...
		private List<Attachment> attachments;// 所包含的附件
	//标记push邮件,区分编码使用
	...
	public Date getSentDate() {
		if (mSentDate == null) {
			...
			*/
			// 发送时间字段为空时优先使用INTERNALDATE，如INTERNALDATE不存在，
			// 再尝试解析Date字段替代，与35 Web Mail保持一致。
			// Modified by LL
			// BEGIN
			mSentDate = getInternalDate();
	...
	public String getContentId() throws MessagingException {
		//return null;
		// Content-ID is very important for handling inline attachments
		// which must be returned correctly!
		// Modified by LL
		// BEGIN
		return getFirstHeader(MimeHeader.HEADER_CONTENT_ID);
		// END
	}
		...
		public void body(BodyDescriptor bd, InputStream in) throws IOException {
			...
			try {
				...
				*/
				// Modified by LL
				Body body = MimeUtility.decodeBody(in,
						bd.getTransferEncoding(), bd.getMimeType(), null,null);
	...
	public void setPushMessage(boolean isPushMessage) {
