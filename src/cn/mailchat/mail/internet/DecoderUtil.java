...
public class DecoderUtil {
    ...
    public static String decodeEncodedWords(String body, Message message) {
        ...
        if (body.indexOf("=?") == -1) {
        	...
        	try {
        		...
        		String  charset  =ParseEncodeUtil.getbyteEncode(b);
        		if(charset==null){
        			charset="ISO-8859-1";
        		}
    ...
    private static String decodeEncodedWord(String body, int begin, int end, Message message) {
        ...
        if (encodedText.isEmpty()) {
            Log.w(MailChat.LOG_TAG, "Missing encoded text in encoded word: '" + body.substring(begin, end) + "'");
            return null;
        }
        ...
        if (encoding.equalsIgnoreCase("Q")) {
            ...
            Log.w(MailChat.LOG_TAG, "Warning: Unknown encoding in encoded word '" + body.substring(begin, end) + "'");
            return null;
        }
    }
    
    public static String decodeString(String body) {
        String ret = body;
        try {
            byte[] bytes = body.getBytes("ISO-8859-1");
            String charset = ParseEncodeUtil.getbyteEncode(bytes);
            if(charset == null || charset.equalsIgnoreCase("US-ASCII")) {
                return body;
            }
