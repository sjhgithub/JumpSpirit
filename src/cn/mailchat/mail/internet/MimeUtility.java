...
package cn.mailchat.mail.internet;
...
import cn.mailchat.mail.*;
import cn.mailchat.mail.Message.RecipientType;
import cn.mailchat.mail.internet.BinaryTempFileBody.BinaryTempFileBodyInputStream;
import cn.mailchat.utils.ParseEncodeUtil;
...
public class MimeUtility {
    ...
    public static String getTextFromPart(Part part) {
        try {
            if ((part != null) && (part.getBody() != null)) {
                ...
                if ((mimeType != null) && MimeUtility.mimeTypeMatches(mimeType, "text/*")) {
                    ...
                    if (charset == null) {
                        ...
                        charset = ParseEncodeUtil.getbyteEncode(bytes);
                    }
             ...
             */
            Log.e(MailChat.LOG_TAG, "Unable to getTextFromPart " + oom.toString());
        } catch (Exception e) {
            /*
             * If we are not able to process the body there's nothing we can do about it. Return
             * null and let the upper layers handle the missing content.
             */
            Log.e(MailChat.LOG_TAG, "Unable to getTextFromPart", e);
        }
        return null;
    }
    ...
    */
    public static Body decodeBody(InputStream in,
            ...
            String contentType,
            DownloadAttachmentNotification notification,Part part)
            throws IOException, MessagingException {
        // We'll remove any transfer encoding by wrapping the stream.
        if (contentTransferEncoding != null) {
            contentTransferEncoding =
    	...
    	//Modified by shengli
    	// BEGIN
        if(contentType.equalsIgnoreCase("text/html")&&part!=null){
            ByteArrayOutputStream outCopy = new ByteArrayOutputStream();
            byte[] buffer = new byte[1024 * 4];
            int n = 0;
            while ((n = in.read(buffer)) != -1) {
                outCopy.write(buffer, 0, n);
            }
            byte[] bufferCopy = outCopy.toByteArray();
            String  charset  =ParseEncodeUtil.getbyteEncode(bufferCopy);
            if(charset==null){
                charset="ISO-8859-1";
            }
            contentType="text/html; charset ="+charset;
                part.setHeader(MimeHeader.HEADER_CONTENT_TYPE, contentType);
                in=new ByteArrayInputStream(bufferCopy);
        }
  		//END
        ...
        tempBody.setEncoding(contentTransferEncoding);
        OutputStream out = tempBody.getOutputStream();
        DownloadCountingOutputStream cos = new DownloadCountingOutputStream(out, notification);
        
        int size = 0;
        try {
            size = IOUtils.copy(in, cos);
        } finally {
            cos.close();
        }
        if (size > 0) {
    ...
    public static class DownloadCountingOutputStream extends CountingOutputStream {
        ...
        private DownloadAttachmentNotification mNotification = null;

        public DownloadCountingOutputStream(OutputStream out,
        		DownloadAttachmentNotification notification) {
            super(out);
        ...
        protected void afterWrite(int n) throws IOException {
            ...
            if (mNotification != null) {
            	// 附件下载被取消
            	if (mNotification.isCanceled.get()) {
            	    throw new IOException(GlobalConstants.CANCELED_BY_USER);
            	}
            	
                mNotification.showProgress(this.getCount());
            }
        }
    }
    ...
    public static ViewableContainer extractTextAndAttachments(Context context, Message message)
        ...
        try {
            ...
            */
        	// 区分text/html和text/plain Parts，不混合排版
        	// Modified by LL
        	// BEGIN
            List<Part> attachments = new ArrayList<Part>();
            ...
            // Used to suppress the divider for the first viewable part
            boolean hideTextDivider = true;
            boolean hideHtmlDivider = true;

            StringBuilder text = new StringBuilder();
            StringBuilder html = new StringBuilder();

            for (Viewable viewable : viewables) {
                if (viewable instanceof Text) {
                    // This is a text/plain part. Fill the variables 'text'.
                    text.append(buildText(viewable, !hideTextDivider));
                    hideTextDivider = false;
                } else if (viewable instanceof Html) {
                	// This is a text/html part. Fill the variables 'html'.
                    html.append(buildHtml(viewable, !hideHtmlDivider));
                    hideHtmlDivider = false;
                } else if (viewable instanceof MessageHeader) {
                    MessageHeader header = (MessageHeader) viewable;
                    Part containerPart = header.getContainerPart();
                    Message innerMessage =  header.getMessage();

                    addTextDivider(text, containerPart, !hideTextDivider);
                    addMessageHeaderText(context, text, innerMessage);

                    addHtmlDivider(html, containerPart, !hideHtmlDivider);
                    addMessageHeaderHtml(context, html, innerMessage);

                    hideTextDivider = true;
                    hideHtmlDivider = true;
                } else if (viewable instanceof Alternative) {
                    // Handle multipart/alternative contents
                    Alternative alternative = (Alternative) viewable;
                    ...
                    // Fill the 'text' variable
                    boolean divider = !hideTextDivider;
                    for (Viewable textViewable : textAlternative) {
                        text.append(buildText(textViewable, divider));
                        divider = true;
                    }
            ...
            if (text.length() == 0 && html.length() != 0) {
            	text.append(HtmlConverter.htmlToText(html.toString()));
            } else if (text.length() != 0 && html.length() == 0) {
            	html.append(HtmlConverter.textToHtml(text.toString()));
            }
    ...
    public static List<Viewable> getViewables(Part part, List<Part> attachments) throws MessagingException {
        ...
        /*
        // 避免格式不正确的text/plain或text/html Parts
        // 被错误地组合在一起当作邮件的文本内容
        // Modified by LL
        // BEGIN
        ...
        // 方案一按 Alternative > Html > Text 的优先级顺序
        // 猜测邮件文本内容，其余格式不正确的Parts被抛弃
        for (Viewable viewable: viewables) {
        	if (viewable instanceof Alternative) {
        		alternative = (Alternative)viewable;
        		if (html != null) {
        			viewables.remove(html);
        			html = null;
        		} else if (text != null) {
        			viewables.remove(text);
        			text = null;
        		}
        	} else if (viewable instanceof Html) {
        		if (text != null) {
        ...
        // 方案二按 Alternative > Html > Text 的优先级顺序
        // 猜测邮件文本内容，其余格式不正确的Parts被当作附件
        // TODO: 规范相应附件名称；正确解码quoted-printable
        for (Viewable viewable: viewables) {
        	if (viewable instanceof Alternative) {
        		alternative = (Alternative)viewable;
        		if (html != null) {
        			attachments.add(html.getPart());
        			viewables.remove(html);
        			html = null;
        		} else if (text != null) {
        			attachments.add(text.getPart());
        			viewables.remove(text);
        			text = null;
        		}
        	} else if (viewable instanceof Html) {
        		...
        		if (alternative == null) {
        			...
        			attachments.add(((Html)viewable).getPart());
        			viewables.remove(viewable);
        		}
        	} else if (viewable instanceof Text) {
        		if (alternative == null && html == null) {
        			...
        			attachments.add(((Text)viewable).getPart());
        			viewables.remove(viewable);
        		}
        	}
        }
    ...
    public static Boolean isPartTextualBody(Part part) throws MessagingException {
        ...
        if (contentType != null) {
        	contentTypeName = MimeUtility.getHeaderParameter(contentType, "name");
        }
    ...
    public static String readToString(InputStream in, String charset) throws IOException {
        ...
        for (String[] rule: CHARSET_FALLBACK_MAP) {
            ...
            if (charset.matches(rule[0])) {
                Log.e(MailChat.LOG_TAG, "I don't know how to deal with the charset " + charset +
    ...
    // Modified by LL
    // BEGIN
    public static long getBase64Size(long input_size) {
    	long adjustment = ( (input_size % 3 != 0) ? (3 - (input_size % 3)) : 0);
    	long code_padded_size = ( (input_size + adjustment) / 3) * 4;
    	long newline_size = ((code_padded_size) / 72) * 2;
    	
    	long total_size = code_padded_size + newline_size;
    	return total_size;
    }
    
    public static long getDecodeBase64Size(long base64_size) {
        return base64_size * 100 / 137;
    }
    // END
    
	public static String getFirstHeader(Part part, String headerName) {
		if (part == null) {
			return null;
		}
		try {
			...
			Log.e(MailChat.LOG_TAG, 
					"Fetch first header of " + headerName + " from " + part + " failed!", e);
			return null;
		}
	}
	
	public static int compareInternalDate(Message messageA, Message messageB) {
		if (messageA != null && messageB != null) {
			Date dateA = messageA.getInternalDate();
			Date dateB = messageB.getInternalDate();
			if (dateA != null && dateB != null) {
			    if (dateA.toString().equals(dateB.toString())) {
			        return 0;
			    } else {
	...
	public static boolean isLocalUid(String uid) {
		if (uid != null) {
			return uid.startsWith(MailChat.LOCAL_UID_PREFIX);
		} else {
			return false;
		}
	}
