...
public class Pop3Store extends Store {
    ...
    class Pop3Folder extends Folder {
                ...
                for (int msgNum = start; msgNum <= end; msgNum++) {
                    ...
                    if (message == null) {
                        ...
                        if (uidParts.length < 3 || !"+OK".equals(uidParts[0])) {
                            Log.e(MailChat.LOG_TAG, "ERR response: " + response);
                            return;
                        }
        ...
        throws MessagingException, IOException {
            ...
            for (String uid : uids) {
                if (mUidToMsgMap.get(uid) == null) {
                    if (MailChat.DEBUG && MailChat.DEBUG_PROTOCOL_POP3) {
                        Log.d(MailChat.LOG_TAG, "Need to index UID " + uid);
                    }
            ...
            while ((response = readLine()) != null) {
                ...
                if (uidParts.length >= 2) {
                    ...
                    if (unindexedUids.contains(msgUid)) {
                        if (MailChat.DEBUG && MailChat.DEBUG_PROTOCOL_POP3) {
                            Log.d(MailChat.LOG_TAG, "Got msgNum " + msgNum + " for UID " + msgUid);
                        }
        ...
        private void indexMessage(int msgNum, Pop3Message message) {
            if (MailChat.DEBUG && MailChat.DEBUG_PROTOCOL_POP3) {
                Log.d(MailChat.LOG_TAG, "Adding index for UID " + message.getUid() + " to msgNum " + msgNum);
            }
        ...
        throws IOException, MessagingException {
            ...
            if (lines != -1 && (!mTopNotSupported || mCapabilities.top)) {
                try {
                    if (MailChat.DEBUG && MailChat.DEBUG_PROTOCOL_POP3 && !mCapabilities.top) {
                        Log.d(MailChat.LOG_TAG, "This server doesn't support the CAPA command. " +
                    ...
                    if (mCapabilities.top) {
                        ...
                        if (MailChat.DEBUG && MailChat.DEBUG_PROTOCOL_POP3) {
                            Log.d(MailChat.LOG_TAG, "The server really doesn't support the TOP " +
                ...
                if (command != null) {
                    if (MailChat.DEBUG && MailChat.DEBUG_PROTOCOL_POP3) {
                        if (sensitive && !MailChat.DEBUG_SENSITIVE) {
                            ...
                            Log.d(MailChat.LOG_TAG, ">>> " + command);
                        }
                    }
...
//         catch (MessagingException me)
//         {
//          Log.w(MailChat.LOG_TAG, "Could not delete non-existent message", me);
//         }
        }
    }
