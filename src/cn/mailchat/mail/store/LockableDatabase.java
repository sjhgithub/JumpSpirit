...
public class LockableDatabase {
    ...
    private class StorageListener implements StorageManager.StorageListener {
        ...
        public void onUnmount(final String providerId) {
            ...
            if (MailChat.DEBUG) {
                Log.d(MailChat.LOG_TAG, "LockableDatabase: Closing DB " + uUid + " due to unmount event on StorageProvider: " + providerId);
            }
            ...
            try {
                ...
                Log.w(MailChat.LOG_TAG, "Unable to writelock on unmount", e);
            }
        }
        ...
        public void onMount(final String providerId) {
            ...
            if (MailChat.DEBUG) {
                Log.d(MailChat.LOG_TAG, "LockableDatabase: Opening DB " + uUid + " due to mount event on StorageProvider: " + providerId);
            }

            try {
                openOrCreateDataspace(mApplication);
            } catch (UnavailableStorageException e) {
                Log.e(MailChat.LOG_TAG, "Unable to open DB on mount", e);
            }
        }
    }
    ...
    public <T> T execute(final boolean transactional, final DbCallback<T> callback) throws UnavailableStorageException {
        ...
        try {
            final boolean debug = MailChat.DEBUG;
            if (doTransaction) {
            ...
            try {
                ...
                if (doTransaction) {
                    ...
                    mDb.endTransaction();
                    if (debug) {
                        Log.v(MailChat.LOG_TAG, "LockableDatabase: Transaction ended, took " + Long.toString(System.currentTimeMillis() - begin) + "ms / " + new Exception().getStackTrace()[1].toString());
                    }
                }
            }
        } finally {
    ...
    public void switchProvider(final String newProviderId) throws MessagingException {
        if (newProviderId.equals(mStorageProviderId)) {
            Log.v(MailChat.LOG_TAG, "LockableDatabase: Ignoring provider switch request as they are equal: " + newProviderId);
            return;
        }
        ...
        try {
            ...
            try {
                try {
                    ...
                    Log.i(MailChat.LOG_TAG, "Unable to close DB on local store migration", e);
                }
    ...
    protected void openOrCreateDataspace(final Application application) throws UnavailableStorageException {
        ...
        try {
            ...
            try {
                ...
                Log.w(MailChat.LOG_TAG, "Unable to open DB " + databaseFile + " - removing file and retrying", e);
                databaseFile.delete();
                if (StorageManager.InternalStorageProvider.ID.equals(mStorageProviderId)) {
                    // internal storage
                    mDb = application.openOrCreateDatabase(databaseFile.getName(), Context.MODE_PRIVATE, null);
    ...
    private void delete(final boolean recreate) throws UnavailableStorageException {
        ...
        try {
            ...
            try {
                ...
                Log.i(MailChat.LOG_TAG, "LockableDatabase: delete(): Unable to delete backing DB file", e);
            }
