    ...
    public ImapResponse readResponse(IImapResponseCallback callback) throws IOException {
        try {
            ...
            if (MailChat.logCollector != null) {
                Log.v(MailChat.LOG_COLLECTOR_TAG, "IMAP Response: " + response);
            }
...
//                    ch == '%' ||
    ...
    private Object parseLiteral() throws IOException {
        ...
        if (mResponse.mCallback != null) {
            ...
            try {
                ...
                mException = e;
                //Log.e(MailChat.LOG_TAG, "parseLiteral(): Exception in callback method", e);
            }
        ...
        //K9默认US-ASCII by shengli
        /**
         * 备注:由于ISO-8859-1是包括ASCII的,所以客户端接受数据时,可以使用ISO-8859-1编码。
         * 再ISO-8859-1可以和中文编码互转,所以用于中间转碼,这样就解决了有些不正规的传输导致的乱码问题(如:直接传输8位GBK或UTF-8编码的中文编码字节流)。
         * 但是,传输是必须使用ASCII标准,因为邮件的传输是7位传输的,ASCII是7位的编码集,所以发邮件时必须使用7位编码。
         */
    ...
    public boolean isStatusResponse(String symbol) {
        return equalsIgnoreCase(symbol, "OK") ||
               equalsIgnoreCase(symbol, "NO") ||
               equalsIgnoreCase(symbol, "BAD") ||
               equalsIgnoreCase(symbol, "PREAUTH") ||
               equalsIgnoreCase(symbol, "BYE");
    }
