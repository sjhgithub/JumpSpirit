...
import cn.mailchat.mail.Authentication;
...
import cn.mailchat.mail.filter.Base64;
import cn.mailchat.mail.filter.EOLConvertingOutputStream;
import cn.mailchat.mail.filter.FixedLengthInputStream;
import cn.mailchat.mail.filter.PeekableInputStream;
...
import cn.mailchat.mail.store.ImapResponseParser.ImapList;
import cn.mailchat.mail.store.ImapResponseParser.ImapResponse;
import cn.mailchat.mail.store.LocalStore.LocalAttachmentBodyPart;
import cn.mailchat.mail.store.imap.ImapUtility;
import cn.mailchat.mail.transport.imap.ImapSettings;
import cn.mailchat.net.ssl.TrustedSocketFactory;
...
public class ImapStore extends Store {
    ...
    private static final String CAPABILITY_PAGING = "C3SEARCH";
    ...
    private ImapConnection mConnection = null;
        ...
        for (ImapResponse response : responses) {
            if (ImapResponseParser.equalsIgnoreCase(response.get(0), commandResponse)) {
                ...
                if (response.size() > 4 || !(response.getObject(3) instanceof String)) {
                    Log.w(MailChat.LOG_TAG, "Skipping incorrectly parsed " + commandResponse +
                            " reply: " + response);
                    continue;
                }
                ...
                try {
                    ...
                    Log.w(MailChat.LOG_TAG, "Folder name not correctly encoded with the UTF-7 variant " +
    ...
    private void autoconfigureFolders(String commandResponse, List<ImapResponse> imapResponses) {
            ...
            if (ImapResponseParser.equalsIgnoreCase(response.get(0), commandResponse)) {
                ...
                for (int i = 0, count = attributes.size(); i < count; i++) {
                    ...
                    if (attribute.equals("\\Drafts")) {
                        ...
                        if (MailChat.DEBUG) Log.d(MailChat.LOG_TAG, "Folder auto-configuration detected draft folder: " + decodedFolderName);
                    } else if (attribute.equals("\\Sent")) {
                        mAccount.setSentFolderName(decodedFolderName);
                        if (MailChat.DEBUG) Log.d(MailChat.LOG_TAG, "Folder auto-configuration detected sent folder: " + decodedFolderName);
                        ...
                        if (MailChat.DEBUG) Log.d(MailChat.LOG_TAG, "Folder auto-configuration detected spam folder: " + decodedFolderName);
                    } else if (attribute.equals("\\Trash")) {
                        mAccount.setTrashFolderName(decodedFolderName);
                        if (MailChat.DEBUG) Log.d(MailChat.LOG_TAG, "Folder auto-configuration detected trash folder: " + decodedFolderName);
                    }
                }
            }
        }
     ...
     * {@link #listFolders(cn.mailchat.mail.store.ImapStore.ImapConnection, boolean)}; we should try to consolidate
     * this at some point. :(
     * @param connection IMAP Connection
     * @throws IOException uh oh!
     * @throws MessagingException uh oh!
     */
    private void autoconfigureFolders(ImapConnection connection) throws IOException, MessagingException {
        String commandResponse = null;
        String commandOptions = null;

        if (connection.capabilities.contains("XLIST")) {
            if (MailChat.DEBUG) Log.d(MailChat.LOG_TAG, "Folder auto-configuration: Using XLIST.");
            ...
            try {
                List<ImapResponse> responses = connection.executeSimpleCommand(
                        String.format("%s%s \"\" %s",
                                ...
                                commandOptions,
                                encodeString(getCombinedPrefix() + "*")));
                autoconfigureFolders(commandResponse, responses);
                return;
            } catch (ImapException e) {
                Log.e(MailChat.LOG_COLLECTOR_TAG, "XLIST 失败", e);
            }
        }

        if(connection.capabilities.contains("SPECIAL-USE")) {
            if (MailChat.DEBUG) Log.d(MailChat.LOG_TAG, "Folder auto-configuration: Using RFC6154/SPECIAL-USE.");
            ...
            try {
                ...
                Log.e(MailChat.LOG_COLLECTOR_TAG, "LIST (SPECIAL-USE) 失败", e);
            }
		}
        ...
        try {
            ...
            Log.e(MailChat.LOG_COLLECTOR_TAG, "LIST 失败", e);
        }

        if (MailChat.DEBUG) Log.d(MailChat.LOG_TAG, "No detected folder auto-configuration methods.");
    }
    ...
    public void checkSettings() throws MessagingException {
        try {
            ...
            throw new MessagingException(MailChat.app.getString(R.string.error_unable_to_connect), ioe);
        }
    }
    ...
    private ImapConnection getConnection() throws MessagingException {
        ...
        synchronized (mConnections) {
            ...
            // 在系统日志中记录IMAP连接获取时的流量计数
            // Modified by LL
            // BEGIN
            Log.v(MailChat.LOG_COLLECTOR_TAG, "Get IMAP " + connection + " IN << " + connection.resetInputCounting());
            Log.v(MailChat.LOG_COLLECTOR_TAG, "Get IMAP " + connection + " OUT >> " + connection.resetOutputCounting());
            // END
        ...
        */
        ...
        // BEGIN
        synchronized (this) {
            ImapConnection connection = mConnection;
            mConnection = null;
            
            if (connection != null) {
                if (connection.isOpen()) {
                    try {
                        connection.setReadTimeout(SOCKET_NOOP_TIMEOUT);
                        connection.executeSimpleCommand("NOOP");
            ...
            if (connection == null) {
                ...
                // 三五3.0邮箱SSL连接需要在登录后获取Capability
                if (mAccount.getVersion_35Mail() == 1) {
                    try {
                        List<ImapResponse> response = connection.executeSimpleCommand(COMMAND_CAPABILITY);
                        connection.receiveCapabilities(response);
                    } catch (Exception e) {
                        Log.e(MailChat.LOG_TAG, "Update capability failed.", e);
                    }
                }
            }
            
            Log.v(MailChat.LOG_COLLECTOR_TAG,
                    "Get IMAP " + getAccount().getEmail()
                    + "/" + connection.getLogId()
                    + " IN << " + connection.resetInputCounting());
            Log.v(MailChat.LOG_COLLECTOR_TAG,
                    "Get IMAP " + getAccount().getEmail()
                    + "/" + connection.getLogId()
                    + " OUT >> " + connection.resetOutputCounting());
            
            return connection;
        }
        // END
        /*
        // 取消连接池，解决网络不稳定情况下，长时间等待超时问题
        // Modified by LL for debug
        // BEGIN
        ImapConnection connection = new ImapConnection(new StoreImapSettings());
    ...
    private void releaseConnection(ImapConnection connection) {
        ...
        */
        ...
        /*
    	// 在系统日志中记录IMAP连接释放时的流量计数
    	// Modified by LL
    	// BEGIN
    	if (connection != null) {
    		Log.v(MailChat.LOG_COLLECTOR_TAG, "Release IMAP " + connection + " IN << " + connection.resetInputCounting());
            Log.v(MailChat.LOG_COLLECTOR_TAG, "Release IMAP " + connection + " OUT >> " + connection.resetOutputCounting());
    	...
    	*/
        ...
        if (connection != null) {
            Log.v(MailChat.LOG_COLLECTOR_TAG,
                    "Release IMAP " + getAccount().getEmail()
                    + "/" + connection.getLogId()
                    + " IN << " + connection.resetInputCounting());
            Log.v(MailChat.LOG_COLLECTOR_TAG,
                    "Release IMAP " + getAccount().getEmail()
    ...
    public boolean isPushCapable() {
        ...
        try {
            ...
            return connection.isIdleCapable();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            releaseConnection(connection);
    ...
    class ImapFolder extends Folder {
        ...
        public String getPrefixedName() throws MessagingException {
			...
			if (!mAccount.getInboxFolderName().equalsIgnoreCase(mName)) {
				...
				// 还原35--2.0邮箱请求邮件前，folider除了收件箱外，其他folder前需要加INBOX.才能取到邮件
				if (mAccount.getVersion_35Mail() == 2
						&& mName.startsWith("INBOX.")) {
					prefixedName = mName;
				} else {
					prefixedName += mName;
					if (mAccount.getVersion_35Mail() == 2
							&& !prefixedName.startsWith("INBOX.")) {
						prefixedName = "INBOX." + prefixedName;
					}
				}
				Log.i(MailChat.LOG_COLLECTOR_TAG,"request folder name ::" + mName);
			} else {
				prefixedName += mName;
			}
            return prefixedName;
        ...
        public List<ImapResponse> internalOpen(int mode) throws MessagingException {
            ...
            try {
                ...
                Log.e(MailChat.LOG_TAG, "Unable to open connection for " + getLogId(), me);
                throw me;
            }
        }
        ...
        public void close() {
            ...
            synchronized (this) {
                ...
                if (mInSearch && mConnection != null) {
                    Log.i(MailChat.LOG_TAG, "IMAP search was aborted, shutting down connection.");
                    mConnection.close();
                } else {
                    releaseConnection(mConnection);
                }
        ...
        public boolean create(FolderType type) throws MessagingException {
            ...
            */
            // Modified by LL for debug
            // BEGIN
            Log.e(MailChat.LOG_COLLECTOR_TAG, "Create IMAP folder " + getName() + " is forbidden");
            return false;
            // END
        }
        ...
        public Map<String, String> copyMessages(Message[] messages, Folder folder)
            ...
            try {
                ...
                if (!exists(remoteDestName)) {
                     ...
                     */
                    if (MailChat.DEBUG) {
                        Log.i(MailChat.LOG_TAG, "ImapFolder.copyMessages: attempting to create remote " +
                        ...
                        if (copyList.size() >= 4 && copyList.getString(0).equals("COPYUID")) {
                            ...
                            if (srcUids != null && destUids != null) {
                                if (srcUids.size() == destUids.size()) {
                                    ...
                                    if (MailChat.DEBUG) {
                                        Log.v(MailChat.LOG_TAG, "Parse error: size of source UIDs " +
                                ...
                                if (MailChat.DEBUG) {
                                    Log.v(MailChat.LOG_TAG, "Parsing of the sequence set failed.");
                                }
                            }
                        }
                    }
        ...
        public void delete(Message[] messages, String trashFolderName) throws MessagingException {
            ...
            if (trashFolderName == null || getName().equalsIgnoreCase(trashFolderName)) {
                ...
                if (!exists(remoteTrashName)) {
                     ...
                     */
                    if (MailChat.DEBUG)
                        Log.i(MailChat.LOG_TAG, "IMAPMessage.delete: attempting to create remote '" + trashFolderName + "' folder for " + getLogId());
                    remoteTrashFolder.create(FolderType.HOLDS_MESSAGES);
                }

                if (exists(remoteTrashName)) {
                    if (MailChat.DEBUG)
                        Log.d(MailChat.LOG_TAG, "IMAPMessage.delete: copying remote " + messages.length + " messages to '" + trashFolderName + "' for " + getLogId());
        ...
        protected long getHighestUid() {
            try {
                ...
                Log.e(MailChat.LOG_TAG, "Unable to find highest UID in folder " + getName(), e);
            }
            return -1L;
        ...
        throws MessagingException {
            ...
            if (fp.contains(FetchProfile.Item.ENVELOPE)) {
                ...
                fetchFields.add("BODY.PEEK[HEADER.FIELDS (date subject from content-type to cc "
						...
						+ MailChat.IDENTITY_HEADER + ")]");
                */
                // 增加提取客户端邮件ID项
                // Modified by LL
                // BEGIN
                /*
                fetchFields.add("BODY.PEEK[HEADER.FIELDS (date subject from content-type to cc bcc "
                		+ "reply-to message-id references in-reply-to "
						+ MailChat.IDENTITY_HEADER + " "
...
//                }
            ...
            if (fp.contains(FetchProfile.Item.EML)) {
                fetchFields.add("BODY[]");
            }
                        ...
                        if (response.mTag == null && ImapResponseParser.equalsIgnoreCase(response.get(1), "FETCH")) {
                            ...
                            if (uid != null) {
                                try {
                                    ...
                                    if (MailChat.DEBUG) {
                                        Log.v(MailChat.LOG_TAG, "Stored uid '" + uid + "' for msgSeq " + msgSeq + " into map " /*+ msgSeqUidMap.toString() */);
                                    }
                                } catch (Exception e) {
                                    Log.e(MailChat.LOG_TAG, "Unable to store uid '" + uid + "' for msgSeq " + msgSeq);
                                }
                            }
                            ...
                            if (message == null) {
                                if (MailChat.DEBUG)
                                    Log.d(MailChat.LOG_TAG, "Do not have message in messageMap for UID " + uid + " for " + getLogId());
                            ...
                            if (literal != null) {
                                if (literal instanceof String) {
                                    ...
                                    if (fp.contains(FetchProfile.Item.EML)) {
                                        String filename = uid + ".eml";
                                        File file = TemporaryAttachmentStore.getFileForWriting(MailChat.app, filename);
                                        ...
                                        //K9取默认编码 by shengli
                                        InputStream bodyStream = new ByteArrayInputStream(bodyString.getBytes("ISO-8859-1"));
...
//                        mAccount.getMaximumAutoDownloadMessageSize());
            ...
            try {
                ...
                // Modified by LL
                // BEGIN
                DownloadAttachmentNotification downloadingNotification = null;
                
                if (part instanceof LocalAttachmentBodyPart) {
                	String downloadingId = ((LocalAttachmentBodyPart)part).getAttachmentId()
                			+ message.getUid() + mAccount.getUuid();
                	synchronized (MailChat.downloadingList) {
    					AttachmentView av = MailChat.downloadingList.get(downloadingId);
    					if (av != null) {
    						downloadingNotification = av.mDownloadingNotification;
    					}
    				}
                }
                
                ImapResponseParser.IImapResponseCallback callback
                	= new FetchPartCallback(part, downloadingNotification);
                // END
                ...
                do {
                    ...
                    if ((response.mTag == null) &&
                        ...
                        if (!message.getUid().equals(uid)) {
                            if (MailChat.DEBUG)
                                Log.d(MailChat.LOG_TAG, "Did not ask for UID " + uid + " for " + getLogId());
                        ...
                        if (literal != null) {
                            if (literal instanceof Body) {
                                ...
                                */
                                // Modified by LL
                                part.setBody(MimeUtility.decodeBody(bodyStream,
                                        contentTransferEncoding, contentType, null, part));
                            } else {
                                // This shouldn't happen
                                throw new MessagingException("Got FETCH response with bogus parameters");
                            }
            ...
            if (fetchList.containsKey("BODYSTRUCTURE")) {
                ...
                if (bs != null) {
                    try {
                        ...
                        if (MailChat.DEBUG)
                            Log.d(MailChat.LOG_TAG, "Error handling message for " + getLogId(), e);
                    ...
                    if (bracketed.size() > 1) {
                        ...
                        if (keyObj instanceof String) {
                            ...
                            if ("UIDNEXT".equalsIgnoreCase(key)) {
                                ...
                                if (MailChat.DEBUG)
                                    Log.d(MailChat.LOG_TAG, "Got UidNext = " + uidNext + " for " + getLogId());
                            }
                        }
                    }
        ...
        protected void handleUntaggedResponse(ImapResponse response) {
            if (response.mTag == null && response.size() > 1) {
                if (ImapResponseParser.equalsIgnoreCase(response.get(1), "EXISTS")) {
                    ...
                    if (MailChat.DEBUG)
                        Log.d(MailChat.LOG_TAG, "Got untagged EXISTS with value " + mMessageCount + " for " + getLogId());
                }
                ...
                if (ImapResponseParser.equalsIgnoreCase(response.get(1), "EXPUNGE") && mMessageCount > 0) {
                    mMessageCount--;
                    if (MailChat.DEBUG)
                        Log.d(MailChat.LOG_TAG, "Got untagged EXPUNGE with mMessageCount " + mMessageCount + " for " + getLogId());
                }
//            if (response.size() > 1) {
//                Object bracketedObj = response.get(1);
//                if (bracketedObj instanceof ImapList)
...
//                                }
//
//                                Log.w(MailChat.LOG_TAG, "ALERT: " + sb.toString() + " for " + getLogId());
//                            }
//                        }
//                    }
//
...
//            }
            ...
            //Log.i(MailChat.LOG_TAG, "mMessageCount = " + mMessageCount + " for " + getLogId());
        }
        ...
        throws MessagingException {
            if (bs.get(0) instanceof ImapList) {
                ...
                int size = -1;
                try {
                    // Parse and save Content-ID which is very important 
                    // for handling in inline attachments.
                    contentId = bs.getString(3);
                    if ("NIL".equalsIgnoreCase(contentId)) {
                        part.removeHeader(MimeHeader.HEADER_CONTENT_ID);
                    } else {
                        part.setHeader(MimeHeader.HEADER_CONTENT_ID, contentId);
                    }
                    encoding = bs.getString(5);
                    size = bs.getNumber(6);
                } catch (NumberFormatException e) {
                    // Parse and save Content-ID which is very important 
                    // for handling in inline attachments.
                    contentId = bs.getString(bs.size() - 7);
                    if ("NIL".equalsIgnoreCase(contentId)) {
                        part.removeHeader(MimeHeader.HEADER_CONTENT_ID);
                    } else {
                        part.setHeader(MimeHeader.HEADER_CONTENT_ID, contentId);
                    }
                    encoding = bs.getString(bs.size() - 5);
                    size = bs.getNumber(bs.size() - 4);
                }
...
//                    [MESSAGE, RFC822, [NAME, Fwd: [#HTR-517941]:  update plans at 1am Friday - Memory allocation - displayware.eml], NIL, NIL, 7BIT, 5974, NIL, [INLINE, [FILENAME*0, Fwd: [#HTR-517941]:  update plans at 1am Friday - Memory all, FILENAME*1, ocation - displayware.eml]], NIL]
                ...
                */
                ...
                if (("text".equalsIgnoreCase(type))
                    ...
                    bodyDisposition = bs.getList(9);
                           ...
                           && (bs.size() > 8)) {
                    if (bs.get(bs.size() - 2) instanceof ImapList) {
                        bodyDisposition = bs.getList(bs.size() - 2);
                    } else if (bs.get(8) instanceof ImapList) {
                ...
                if (size > 0 && (part.getBody() instanceof BinaryTempFileBody)) {
                    BinaryTempFileBody body = (BinaryTempFileBody) part.getBody();
                    if (body.getSize() == 0) {
                        part.setBody(null);
                    }
                }
        ...
        public Map<String, String> appendMessages(Message[] messages) throws MessagingException {
            ...
            try {
                ...
                for (Message message : messages) {
                    ...
                    // TODO: mail.35.cn无法返回正确的UID，查找原因
                    // Modified by LL for debug
                    String newUid = MailChat.LOCAL_UID_PREFIX + UUID.randomUUID();
                    
                    if (MailChat.DEBUG) {
                        Log.d(MailChat.LOG_TAG, "Got UID " + newUid + " for message for " + getLogId());
                    }
            ...
            try {
                ...
                */
                ...
                if (messageIdHeader == null || messageIdHeader.length == 0) {
                    if (MailChat.DEBUG)
                        Log.d(MailChat.LOG_TAG, "Did not get a message-id in order to search for UID  for " + getLogId());
                    return null;
                }
                String messageId = messageIdHeader[0];
                if (MailChat.DEBUG)
                    Log.d(MailChat.LOG_TAG, "Looking for UID for message with message-id " + messageId + " for " + getLogId());
        ...
        public String getNewPushState(String oldPushStateS, Message message) {
            try {
                ...
                Log.e(MailChat.LOG_TAG, "Exception while updated push state for " + getLogId(), e);
                return null;
            }
        }
        ...
        private MessagingException ioExceptionHandler(ImapConnection connection, IOException ioe) {
            Log.e(MailChat.LOG_TAG, "IOException for " + getLogId(), ioe);
            if (connection != null) {
                connection.close();
            }
            close();
        ...
        public List<Message> search(final String queryString, final Flag[] requiredFlags, final Flag[] forbiddenFlags)
            ...
            if (ImapStore.this.mConnection != null && ImapStore.this.mConnection.hasCapability(CAPABILITY_PAGING)) {
                MailChat.remoteSearchPaging = true;
                MailChat.remoteSearchQuery = queryString;
            }
            ...
            final ImapSearcher searcher = new ImapSearcher() {
                public List<ImapResponse> search() throws IOException, MessagingException {
                    ...
                    String searchRange;
                    switch(MailChat.remoteSearchRange) {
                    case SENDER:
                        searchRange = "FROM {%1$d}\r\n" + queryString;
                        break;
                    case RECIPIENTS:
                        searchRange = "OR CC {%1$d}\r\n" + queryString
                            + " OR BCC {%1$d}\r\n" + queryString
                            + " TO {%1$d}\r\n" + queryString;
                        break;
                    case SUBJECT:
                        searchRange = "SUBJECT {%1$d}\r\n" + queryString;
                        break;
                    case TEXT:
                        searchRange = "TEXT {%1$d}\r\n" + queryString;
                        break;
                    default:
                        searchRange = "TEXT {%1$d}\r\n" + queryString;
                    }
                    ...
                    if (MailChat.remoteSearchPaging) {
                        pagingString = ":f" + MailChat.remoteSearchStart
                                + " :s" + MailChat.remoteSearchSize;
                        searchRange = searchRange
                                + " TEXT {%2$d}\r\n" + pagingString;
                    }
                    
                    if (mAccount.getVersion_35Mail() == 2) {
                        int queryStringLength = queryString.getBytes("GB2312").length;
                        int pagingStringLength = pagingString.getBytes("GB2312").length;
                        imapQuery += String.format("CHARSET GB2312 " + searchRange,
                                queryStringLength,
                                pagingStringLength);
                    } else {
                        int queryStringLength = queryString.getBytes().length;
                        int pagingStringLength = pagingString.getBytes().length;
                        imapQuery += String.format("CHARSET UTF-8 " + searchRange,
                                queryStringLength,
                                pagingStringLength);
                    }
		...
		public boolean isFlagSupported(Flag flag) {
			if (mCanCreateKeywords || mPermanentFlagsIndex.contains(flag)) {
				return true;
			} else {
				return false;
			}
    ...
    public static class ImapConnection {
        ...
        private CountingInputStream mCountingIn;
        private CountingOutputStream mCountingOut;
        ...
        private List<ImapResponse> receiveCapabilities(List<ImapResponse> responses) {
            for (ImapResponse response : responses) {
                ...
                if (capabilityList != null && !capabilityList.isEmpty() &&
                    ...
                    if (MailChat.DEBUG) {
                        Log.d(MailChat.LOG_TAG, "Saving " + capabilityList.size() + " capabilities for " + getLogId());
                    }
                    for (Object capability : capabilityList) {
                        if (capability instanceof String) {
//                            if (MailChat.DEBUG)
//                            {
//                                Log.v(MailChat.LOG_TAG, "Saving capability '" + capability + "' for " + getLogId());
//                            }
        ...
        public void open() throws IOException, MessagingException {
            ...
            try {
                ...
                Log.w(MailChat.LOG_TAG, "Could not set DNS ttl to 0 for " + getLogId(), e);
            }
            ...
            try {
                ...
                Log.w(MailChat.LOG_TAG, "Could not set DNS negative ttl to 0 for " + getLogId(), e);
            }
            ...
            try {
                ...
                for (int i = 0; i < addresses.length; i++) {
                    try {
                        if (MailChat.DEBUG && MailChat.DEBUG_PROTOCOL_IMAP) {
                            Log.d(MailChat.LOG_TAG, "Connecting to " + mSettings.getHost() + " as " +
                                    addresses[i]);
                        }
                ...
                */
                if (mCountingIn != null) {
                	Log.i(MailChat.LOG_TAG, "New IMAP Connection is open. IN<<" + mCountingIn.getByteCount());
                }
                if (mCountingOut != null) {
                	Log.i(MailChat.LOG_TAG, "New IMAP Connection is open. OUT>>" + mCountingOut.getByteCount());
                }
                
                mCountingIn = new CountingInputStream(
                		new BufferedInputStream(
                				mSocket.getInputStream(),
                				8192));
                mIn = new PeekableInputStream(mCountingIn);
                
				mParser = new ImapResponseParser(mIn);
				
				mCountingOut = new CountingOutputStream(mSocket.getOutputStream());
				mOut = new BufferedOutputStream(mCountingOut, 8192);

                capabilities.clear();
                ImapResponse nullResponse = mParser.readResponse();
                if (MailChat.DEBUG && MailChat.DEBUG_PROTOCOL_IMAP)
                    Log.v(MailChat.LOG_TAG, getLogId() + "<<<" + nullResponse);
                ...
                if (!hasCapability(CAPABILITY_CAPABILITY)) {
                    if (MailChat.DEBUG)
                        Log.i(MailChat.LOG_TAG, "Did not get capabilities in banner, requesting CAPABILITY for " + getLogId());
                ...
                if (hasCapability(CAPABILITY_ID)) {
                    try {
                        executeSimpleCommand("ID (\"name\" \"MailChat\""
                                + " \"version\" \"" + MailChat.app.getString(R.string.version_name) + "\""
                                + " \"os\" \"Android\""
                                + " \"vendor\" \"Youqia Technology Co., Ltd.\""
                                + " \"contact\" \"fb@mailchat.cn\")");
                    } catch (Exception e) {
                        Log.e(MailChat.LOG_TAG, "Unable to exchange ID", e);
                    }
                }
                ...
                if (mSettings.getConnectionSecurity() == ConnectionSecurity.STARTTLS_REQUIRED) {
                    ...
                    if (hasCapability("STARTTLS")) {
                        ...
                        if (MailChat.DEBUG)
                            Log.i(MailChat.LOG_TAG, "Updating capabilities after STARTTLS for " + getLogId());
                        capabilities.clear();
                        List<ImapResponse> responses = receiveCapabilities(executeSimpleCommand(COMMAND_CAPABILITY));
                        if (responses.size() != 2) {
                            throw new MessagingException("Invalid CAPABILITY response received");
                ...
                case EXTERNAL:
                    if (hasCapability(CAPABILITY_AUTH_EXTERNAL)) {
                        ...
                        throw new CertificateValidationException(MailChat.app.getString(R.string.auth_external_error));
                    }
                    break;
                ...
                if (MailChat.DEBUG) {
                    Log.d(MailChat.LOG_TAG, CAPABILITY_COMPRESS_DEFLATE + " = " + hasCapability(CAPABILITY_COMPRESS_DEFLATE));
                }
                if (hasCapability(CAPABILITY_COMPRESS_DEFLATE)) {
                    ConnectivityManager connectivityManager = (ConnectivityManager)MailChat.app.getSystemService(Context.CONNECTIVITY_SERVICE);
                    ...
                    if (netInfo != null) {
                        ...
                        if (MailChat.DEBUG)
                            Log.d(MailChat.LOG_TAG, "On network type " + type);
                        useCompression = mSettings.useCompression(type);

                    }
                    if (MailChat.DEBUG)
                        Log.d(MailChat.LOG_TAG, "useCompression " + useCompression);
                    if (useCompression) {
                        try {
                            ...
                            if (MailChat.DEBUG) {
                                Log.i(MailChat.LOG_TAG, "Compression enabled for " + getLogId());
                            }
                        } catch (Exception e) {
                            Log.e(MailChat.LOG_TAG, "Unable to negotiate compression", e);
                        }
                    }
                }
                ...
                if (MailChat.DEBUG)
                    Log.d(MailChat.LOG_TAG, "NAMESPACE = " + hasCapability(CAPABILITY_NAMESPACE)
                          + ", mPathPrefix = " + mSettings.getPathPrefix());

                if (mSettings.getPathPrefix() == null) {
                    if (hasCapability(CAPABILITY_NAMESPACE)) {
                        if (MailChat.DEBUG)
                            Log.i(MailChat.LOG_TAG, "mPathPrefix is unset and server has NAMESPACE capability");
                        ...
                        for (ImapResponse response : namespaceResponses) {
                            if (ImapResponseParser.equalsIgnoreCase(response.get(0), COMMAND_NAMESPACE)) {
                                if (MailChat.DEBUG)
                                    Log.d(MailChat.LOG_TAG, "Got NAMESPACE response " + response + " on " + getLogId());

                                Object personalNamespaces = response.get(1);
                                if (personalNamespaces != null && personalNamespaces instanceof ImapList) {
                                    if (MailChat.DEBUG)
                                        Log.d(MailChat.LOG_TAG, "Got personal namespaces: " + personalNamespaces);
                                    ...
                                    if (firstNamespace != null && firstNamespace instanceof ImapList) {
                                        if (MailChat.DEBUG)
                                            Log.d(MailChat.LOG_TAG, "Got first personal namespaces: " + firstNamespace);
                                        ...
                                        if (MailChat.DEBUG)
                                            Log.d(MailChat.LOG_TAG, "Got path '" + mSettings.getPathPrefix() + "' and separator '" + mSettings.getPathDelimeter() + "'");
                                    }
                                }
                            }
                        }
                        ...
                        if (MailChat.DEBUG)
                            Log.i(MailChat.LOG_TAG, "mPathPrefix is unset but server does not have NAMESPACE capability");
                    ...
                    try {
                        ...
                        for (ImapResponse response : nameResponses) {
                            if (ImapResponseParser.equalsIgnoreCase(response.get(0), "LIST")) {
                                ...
                                mSettings.setCombinedPrefix(null);
                                if (MailChat.DEBUG)
                                    Log.d(MailChat.LOG_TAG, "Got path delimeter '" + mSettings.getPathDelimeter() + "' for " + getLogId());
                            }
                        }
                    } catch (Exception e) {
                        Log.e(MailChat.LOG_TAG, "Unable to get path delimeter using LIST", e);
                    }
                }

            } catch (SSLException e) {
				// throw new CertificateValidationException(e.getMessage(), e);
				// 修复所有SSL异常都抛出CertificateValidation异常造成的误报认证错误问题
                ...
                if (tokens != null && tokens.length > 1 && tokens[1] != null) {
                    Log.e(MailChat.LOG_TAG, "Stripping host/port from ConnectionException for " + getLogId(), ce);
                ...
                if (!authSuccess) {
                    Log.e(MailChat.LOG_TAG, "Failed to login, closing connection for " + getLogId());
                    close();
                }
            }
        }
            ...
            do {
                ...
                if (response.mTag != null) {
                    if (response.mTag.equalsIgnoreCase(tag)) {
                        ...
                        Log.w(MailChat.LOG_TAG, "After sending tag " + tag
        ...
        protected ArrayList<ImapResponse> readStatusResponse(String tag,
            ...
            do {
                ...
                if (MailChat.DEBUG && MailChat.DEBUG_PROTOCOL_IMAP)
                    Log.v(MailChat.LOG_TAG, getLogId() + "<<<" + response);

                if (response.mTag != null && !response.mTag.equalsIgnoreCase(tag)) {
                    Log.w(MailChat.LOG_TAG, "After sending tag " + tag + ", got tag response from previous command " + response + " for " + getLogId());
        ...
        protected boolean isIdleCapable() {
            if (MailChat.DEBUG)
                Log.v(MailChat.LOG_TAG, "Connection " + getLogId() + " has " + capabilities.size() + " capabilities");
        ...
        public void sendContinuation(String continuation) throws IOException {
            ...
            if (MailChat.DEBUG && MailChat.DEBUG_PROTOCOL_IMAP)
                Log.v(MailChat.LOG_TAG, getLogId() + ">>> " + continuation);
        ...
        throws MessagingException, IOException {
            try {
                ...
                // Generate Foxmail style command tag for Netease mailboxes 
                // (163.com, 126.com, yeah.net etc.) compatibility.
                // Modified by LL
                // BEGIN
                String tag = "C" + Integer.toString(mNextCommandTag++);
                // END
                
                String commandToSend = tag + " " + command + "\r\n";
                if (command.startsWith("UID SEARCH CHARSET GB2312 ")) {
                    mOut.write(commandToSend.getBytes("GB2312"));
                } else {
                    mOut.write(commandToSend.getBytes());
                }
                mOut.flush();
                if (MailChat.DEBUG && MailChat.DEBUG_PROTOCOL_IMAP) {
                    if (sensitive && !MailChat.DEBUG_SENSITIVE) {
                        Log.v(MailChat.LOG_TAG, getLogId() + ">>> "
                              + "[Command Hidden, Enable Sensitive Debug Logging To Show]");
                    } else {
                        Log.v(MailChat.LOG_TAG, getLogId() + ">>> " + commandToSend);
                    }
                }
                
                // 在系统日志中记录发送的IMAP命令，隐藏敏感指令
                // Modified by LL
                // BEGIN
                if (sensitive) {
                    Log.v(MailChat.LOG_COLLECTOR_TAG,
                            "IMAP Command: " + tag + " ***SENSITIVE***\r\n" );
                } else {
                    Log.v(MailChat.LOG_COLLECTOR_TAG,
                            "IMAP Command: " + commandToSend);
                }
                // END
        ...
        throws IOException, ImapException, MessagingException {
            ...
            //if (MailChat.DEBUG)
            //    Log.v(MailChat.LOG_TAG, "Sending IMAP command " + commandToLog + " on connection " + getLogId());

            String tag = sendCommand(command, sensitive);
            //if (MailChat.DEBUG)
            //    Log.v(MailChat.LOG_TAG, "Sent IMAP command " + commandToLog + " with tag " + tag + " for " + getLogId());
        ...
        // Modified by LL
        // BEGIN
        public long resetInputCounting() {
        	if (mCountingIn != null) {
        		return mCountingIn.resetByteCount();
        	} else {
        		return -1;
        	}
        }
        
        public long resetOutputCounting() {
        	if (mCountingOut != null) {
        		return mCountingOut.resetByteCount();
        	} else {
        		return -1;
        	}
        }
    ...
    public class ImapFolderPusher extends ImapFolder implements UntaggedHandler {
        ...
        public void refresh() throws IOException, MessagingException {
            if (idling.get()) {
                wakeLock.acquire(MailChat.PUSH_WAKE_LOCK_TIMEOUT);
                sendDone();
            }
        }
        ...
        public void start() {
            Runnable runner = new Runnable() {
                public void run() {
                    wakeLock.acquire(MailChat.PUSH_WAKE_LOCK_TIMEOUT);
                    if (MailChat.DEBUG)
                        Log.i(MailChat.LOG_TAG, "Pusher starting for " + getLogId());
                    ...
                    while (!stop.get()) {
                        try {
                            ...
                            try {
                                ...
                                if (MailChat.DEBUG)
                                    Log.i(MailChat.LOG_TAG, "Got oldUidNext " + oldUidNext + " for " + getLogId());
                            } catch (Exception e) {
                                Log.e(MailChat.LOG_TAG, "Unable to get oldUidNext for " + getLogId(), e);
                            }
                            ...
                            */
                            // 2.0邮箱反复刷新问题，可能和IDLE指令有关
                            // TODO 确定原因以彻底解决
                            // Modified by LL for debug
                            if (!conn.isIdleCapable() || ImapFolderPusher.this.getAccount().getVersion_35Mail() == 2) {
                                stop.set(true);
                            ...
                            if (newUidNext == -1) {
                                if (MailChat.DEBUG) {
                                    Log.d(MailChat.LOG_TAG, "uidNext is -1, using search to find highest UID");
                                }
                                long highestUid = getHighestUid();
                                if (highestUid != -1L) {
                                    if (MailChat.DEBUG)
                                        Log.d(MailChat.LOG_TAG, "highest UID = " + highestUid);
                            ...
                            if (newUidNext > startUid) {
                                ...
                                if (MailChat.DEBUG)
                                    Log.i(MailChat.LOG_TAG, "Needs sync from uid " + startUid  + " to " + newUidNext + " for " + getLogId());
                                ...
                                while (!storedUntaggedResponses.isEmpty()) {
                                    if (MailChat.DEBUG)
                                        Log.i(MailChat.LOG_TAG, "Processing " + storedUntaggedResponses.size() + " untagged responses from previous commands for " + getLogId());
                                ...
                                if (MailChat.DEBUG)
                                    Log.i(MailChat.LOG_TAG, "About to IDLE for " + getLogId());
                            ...
                            try {
                                ...
                                Log.e(MailChat.LOG_TAG, "Got exception while closing for exception for " + getLogId(), me);
                            }
                            if (stop.get()) {
                                Log.i(MailChat.LOG_TAG, "Got exception while idling, but stop is set for " + getLogId());
                            } else {
                                receiver.pushError("Push error for " + getName(), e);
                                Log.e(MailChat.LOG_TAG, "Got exception while idling for " + getLogId(), e);
                                ...
                                if (idleFailureCount.incrementAndGet() > IDLE_FAILURE_COUNT_LIMIT) {
                                    Log.e(MailChat.LOG_TAG, "Disabling pusher for " + getLogId() + " after " + idleFailureCount.get() + " consecutive errors");
                    ...
                    try {
                        if (MailChat.DEBUG)
                            Log.i(MailChat.LOG_TAG, "Pusher for " + getLogId() + " is exiting");
                        close();
                    } catch (Exception me) {
                        Log.e(MailChat.LOG_TAG, "Got exception while closing for " + getLogId(), me);
                    } finally {
                        wakeLock.release();
                    }
                }
        ...
        protected void handleUntaggedResponse(ImapResponse response) {
            if (response.mTag == null && response.size() > 1) {
                ...
                if (ImapResponseParser.equalsIgnoreCase(responseType, "FETCH")
                    ...
                    if (MailChat.DEBUG)
                        Log.d(MailChat.LOG_TAG, "Storing response " + response + " for later processing");
        ...
        protected void processUntaggedResponses(List<ImapResponse> responses) throws MessagingException {
            ...
            if (MailChat.DEBUG)
                Log.d(MailChat.LOG_TAG, "UIDs for messages needing flag sync are " + flagSyncMsgSeqs + "  for " + getLogId());
        ...
        private void syncMessages(int end, boolean newArrivals) throws MessagingException {
            ...
            if (messageArray != null && messageArray.length > 0) {
                ...
                if (MailChat.DEBUG)
                    Log.i(MailChat.LOG_TAG, "Got newUid " + newUid + " for message " + end + " on " + getLogId());
                long startUid = oldUidNext;
                ...
                if (newUid >= startUid) {
                    ...
                    if (MailChat.DEBUG)
                        Log.i(MailChat.LOG_TAG, "Needs sync from uid " + startUid  + " to " + newUid + " for " + getLogId());
                    List<Message> messages = new ArrayList<Message>();
        ...
        private void removeMessages(List<String> removeUids) {
            ...
            try {
                ...
                for (Message existingMessage : existingMessages) {
                    ...
                    Log.w(MailChat.LOG_TAG, "Message with UID " + existingUid + " still exists on server, not expunging");
                ...
                for (String uid : removeUids) {
                    ...
                    try {
                        ...
                        Log.e(MailChat.LOG_TAG, "Unable to set DELETED flag on message " + message.getUid());
                    }
                    messages.add(message);
                }
                receiver.messagesRemoved(this, messages);
            } catch (Exception e) {
                Log.e(MailChat.LOG_TAG, "Cannot remove EXPUNGEd messages", e);
            }
        ...
        protected int processUntaggedResponse(long oldMessageCount, ImapResponse response, List<Long> flagSyncMsgSeqs, List<String> removeMsgUids) {
            ...
            if (response.mTag == null && response.size() > 1) {
                try {
                    Object responseType = response.get(1);
                    if (ImapResponseParser.equalsIgnoreCase(responseType, "FETCH")) {
                        Log.i(MailChat.LOG_TAG, "Got FETCH " + response);
                        long msgSeq = response.getLong(0);

                        if (MailChat.DEBUG)
                            Log.d(MailChat.LOG_TAG, "Got untagged FETCH for msgseq " + msgSeq + " for " + getLogId());
                    ...
                    if (ImapResponseParser.equalsIgnoreCase(responseType, "EXPUNGE")) {
                        ...
                        if (MailChat.DEBUG)
                            Log.d(MailChat.LOG_TAG, "Got untagged EXPUNGE for msgseq " + msgSeq + " for " + getLogId());
                        ...
                        for (long msgSeqNum : msgSeqs) {
                            if (MailChat.DEBUG) {
                                Log.v(MailChat.LOG_TAG, "Comparing EXPUNGEd msgSeq " + msgSeq + " to " + msgSeqNum);
                            }
                            if (msgSeqNum == msgSeq) {
                                String uid = msgSeqUidMap.get(msgSeqNum);
                                if (MailChat.DEBUG) {
                                    Log.d(MailChat.LOG_TAG, "Scheduling removal of UID " + uid + " because msgSeq " + msgSeqNum + " was expunged");
                                }
                                ...
                                String uid = msgSeqUidMap.get(msgSeqNum);
                                if (MailChat.DEBUG) {
                                    Log.d(MailChat.LOG_TAG, "Reducing msgSeq for UID " + uid + " from " + msgSeqNum + " to " + (msgSeqNum - 1));
                                }
                                msgSeqUidMap.remove(msgSeqNum);
                    ...
                    Log.e(MailChat.LOG_TAG, "Could not handle untagged FETCH for " + getLogId(), e);
                }
            }
        ...
        public void stop() {
            ...
            if (conn != null) {
                if (MailChat.DEBUG)
                    Log.v(MailChat.LOG_TAG, "Closing mConnection to stop pushing for " + getLogId());
                conn.close();
            } else {
                Log.w(MailChat.LOG_TAG, "Attempt to interrupt null mConnection to stop pushing on folderPusher for " + getLogId());
            }
        }

        public void handleAsyncUntaggedResponse(ImapResponse response) {
            if (MailChat.DEBUG)
                Log.v(MailChat.LOG_TAG, "Got async response: " + response);

            if (stop.get()) {
                if (MailChat.DEBUG)
                    Log.d(MailChat.LOG_TAG, "Got async untagged response: " + response + ", but stop is set for " + getLogId());

                try {
                    sendDone();
                } catch (Exception e) {
                    Log.e(MailChat.LOG_TAG, "Exception while sending DONE for " + getLogId(), e);
                }
            } else {
                if (response.mTag == null) {
                    if (response.size() > 1) {
                        ...
                        if (ImapResponseParser.equalsIgnoreCase(responseType, "EXISTS") || ImapResponseParser.equalsIgnoreCase(responseType, "EXPUNGE") ||
                            ...
                            if (MailChat.DEBUG)
                                Log.d(MailChat.LOG_TAG, "Got useful async untagged response: " + response + " for " + getLogId());
                        ...
                        if (MailChat.DEBUG)
                            Log.d(MailChat.LOG_TAG, "Idling " + getLogId());
    ...
    public class ImapPusher implements Pusher {
            ...
            synchronized (folderPushers) {
                for (ImapFolderPusher folderPusher : folderPushers.values()) {
                    try {
                        ...
                        Log.e(MailChat.LOG_TAG, "Got exception while refreshing for " + folderPusher.getName(), e);
                    }
                }
            }
        }
        ...
        public void stop() {
            if (MailChat.DEBUG)
                Log.i(MailChat.LOG_TAG, "Requested stop of IMAP pusher");

            synchronized (folderPushers) {
                for (ImapFolderPusher folderPusher : folderPushers.values()) {
                    try {
                        if (MailChat.DEBUG)
                            Log.i(MailChat.LOG_TAG, "Requesting stop of IMAP folderPusher " + folderPusher.getName());
                        folderPusher.stop();
                    } catch (Exception e) {
                        Log.e(MailChat.LOG_TAG, "Got exception while stopping " + folderPusher.getName(), e);
                    }
                }
                folderPushers.clear();
            }
                    ...
                    if (thisState.hasMoreTokens()) {
                        ...
                        if ("uidNext".equalsIgnoreCase(key) && thisState.hasMoreTokens()) {
                            ...
                            try {
                                ...
                                Log.e(MailChat.LOG_TAG, "Unable to part uidNext value " + value, e);
                            }
    ...
    private static class FetchPartCallback implements ImapResponseParser.IImapResponseCallback {
        private Part mPart;
        private DownloadAttachmentNotification mNotification;

        FetchPartCallback(Part part, DownloadAttachmentNotification notification) {
            mPart = part;
        ...
        public Object foundLiteral(ImapResponse response,
            ...
            if (response.mTag == null
                ...
                return MimeUtility.decodeBody(literal, contentTransferEncoding,
                        contentType, mNotification,mPart);
            }
            return null;
        }
    }
