...
public class IndexView extends View {
	...
	public IndexView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		//底色
		paint.setColor(a.getColor(1, 0xffffff));
	...
	public void draw(Canvas canvas) {
		...
		if (!isInitItemHeight) {
			...
			offsetY = (int) ((itemHeight - (fontMetrics.bottom - fontMetrics.top))
					* 0.5 - fontMetrics.top + getPaddingTop());
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			//下压色
			setBackgroundColor(0xe5e5e5);
		case MotionEvent.ACTION_MOVE:
			final float y = event.getY();
			final int newIndex = (int) (y / itemHeight);
			if (lastIndex != newIndex && 0 <= newIndex
					&& newIndex < indexLetters.length) {
				lastIndex = newIndex;
