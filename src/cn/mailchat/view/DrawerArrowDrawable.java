...
public abstract class DrawerArrowDrawable extends Drawable {
	...
	protected boolean isDrowUnreadIcon = false;
	protected int mSize;
	...
	public DrawerArrowDrawable(Context context) {
		...
		this.mPaint.setColor(context.getResources().getColor(
				R.color.message_title_view_sub_gray));
		...
		DrawerCircleDrawable(context);
	}

	public void DrawerCircleDrawable(Context context) {
		this.context = context;
	...
	public void draw(Canvas canvas) {
		...
		this.mPath.close();
		if (isDrowUnreadIcon) {
			canvas.drawCircle(mBarSize + localRect.centerX(),
					localRect.centerY(), 10, mCirclePaint);// 小圆
		}
		canvas.save();
		if (!isLayoutRtl())
	...
	public void setDrowUnreadCircleIcon(boolean isDrowUnreadIcon) {
		this.isDrowUnreadIcon = isDrowUnreadIcon;
		invalidateSelf();
	}
}