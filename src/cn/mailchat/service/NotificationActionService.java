...
public class NotificationActionService extends CoreService {
    private final static String REPLY_ACTION = "cn.mailchat.service.NotificationActionService.REPLY_ACTION";
    private final static String READ_ALL_ACTION = "cn.mailchat.service.NotificationActionService.READ_ALL_ACTION";
    private final static String DELETE_ALL_ACTION = "cn.mailchat.service.NotificationActionService.DELETE_ALL_ACTION";
    private final static String ACKNOWLEDGE_ACTION = "cn.mailchat.service.NotificationActionService.ACKNOWLEDGE_ACTION";
    ...
    public int startService(Intent intent, int startId) {
        if (MailChat.DEBUG)
            Log.i(MailChat.LOG_TAG, "NotificationActionService started with startId = " + startId);
        ...
        if (account != null) {
            if (READ_ALL_ACTION.equals(action)) {
                if (MailChat.DEBUG)
                    Log.i(MailChat.LOG_TAG, "NotificationActionService marking messages as read");
                ...
                if (MailChat.DEBUG)
                    Log.i(MailChat.LOG_TAG, "NotificationActionService deleting messages");
                ...
                controller.deleteMessages(messages,true, null);
            } else if (REPLY_ACTION.equals(action)) {
                if (MailChat.DEBUG)
                    Log.i(MailChat.LOG_TAG, "NotificationActionService initiating reply");
                ...
                if (message != null) {
                    Intent i = MailComposeActivity.getActionReplyIntent(this, account, message, false, null);
                    i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    startActivity(i);
                } else {
                    Log.i(MailChat.LOG_TAG, "Could not execute reply action.");
                }
            ...
            Log.w(MailChat.LOG_TAG, "Could not find account for notification action.");
        }
