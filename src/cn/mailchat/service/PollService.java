...
public class PollService extends CoreService {
    private static String START_SERVICE = "cn.mailchat.service.PollService.startService";
    private static String STOP_SERVICE = "cn.mailchat.service.PollService.stopService";
    ...
    public int startService(Intent intent, int startId) {
        if (START_SERVICE.equals(intent.getAction())) {
            if (MailChat.DEBUG)
                Log.i(MailChat.LOG_TAG, "PollService started with startId = " + startId);

            MessagingController controller = MessagingController.getInstance(getApplication());
            Listener listener = (Listener)controller.getCheckMailListener();
            if (listener == null) {
                if (MailChat.DEBUG)
                    Log.i(MailChat.LOG_TAG, "***** PollService *****: starting new check");
                ...
                if (MailChat.DEBUG)
                    Log.i(MailChat.LOG_TAG, "***** PollService *****: renewing WakeLock");
                listener.setStartId(startId);
                listener.wakeLockAcquire();
            }
        } else if (STOP_SERVICE.equals(intent.getAction())) {
            if (MailChat.DEBUG)
                Log.i(MailChat.LOG_TAG, "PollService stopping");
            stopSelf();
        }
    ...
    class Listener extends MessagingListener {
        ...
        private void release() {
            ...
            wakeLockRelease();
            if (MailChat.DEBUG)
                Log.i(MailChat.LOG_TAG, "PollService stopping with startId = " + startId);
        ...
        public void checkMailFinished(Context context, Account account) {
            ...
            if (MailChat.DEBUG)
                Log.v(MailChat.LOG_TAG, "***** PollService *****: checkMailFinished");
            release();
        }
