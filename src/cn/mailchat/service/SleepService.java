...
public class SleepService extends CoreService {
    ...
    private static String ALARM_FIRED = "cn.mailchat.service.SleepService.ALARM_FIRED";
    private static String LATCH_ID = "cn.mailchat.service.SleepService.LATCH_ID_EXTRA";
    ...
    public static void sleep(Context context, long sleepTime, TracingWakeLock wakeLock, long wakeLockTimeout) {
        ...
        if (MailChat.DEBUG)
            Log.d(MailChat.LOG_TAG, "SleepService Preparing CountDownLatch with id = " + id + ", thread " + Thread.currentThread().getName());
        ...
        Intent i = new Intent();
        i.setClassName(context.getPackageName(), "cn.mailchat.service.SleepService");
        ...
        try {
            ...
            if (!countedDown) {
                if (MailChat.DEBUG)
                    Log.d(MailChat.LOG_TAG, "SleepService latch timed out for id = " + id + ", thread " + Thread.currentThread().getName());
            }
        } catch (InterruptedException ie) {
            Log.e(MailChat.LOG_TAG, "SleepService Interrupted while awaiting latch", ie);
        }
        SleepDatum releaseDatum = sleepData.remove(id);
        if (releaseDatum == null) {
            try {
                if (MailChat.DEBUG)
                    Log.d(MailChat.LOG_TAG, "SleepService waiting for reacquireLatch for id = " + id + ", thread " + Thread.currentThread().getName());
                if (!sleepDatum.reacquireLatch.await(5000, TimeUnit.MILLISECONDS)) {
                    Log.w(MailChat.LOG_TAG, "SleepService reacquireLatch timed out for id = " + id + ", thread " + Thread.currentThread().getName());
                } else if (MailChat.DEBUG)
                    Log.d(MailChat.LOG_TAG, "SleepService reacquireLatch finished for id = " + id + ", thread " + Thread.currentThread().getName());
            } catch (InterruptedException ie) {
                Log.e(MailChat.LOG_TAG, "SleepService Interrupted while awaiting reacquireLatch", ie);
            }
        } else {
        ...
        if (actualSleep < sleepTime) {
            Log.w(MailChat.LOG_TAG, "SleepService sleep time too short: requested was " + sleepTime + ", actual was " + actualSleep);
        } else {
            if (MailChat.DEBUG)
                Log.d(MailChat.LOG_TAG, "SleepService requested sleep time was " + sleepTime + ", actual was " + actualSleep);
        }
    }
    ...
    private static void endSleep(Integer id) {
        if (id != -1) {
            ...
            if (sleepDatum != null) {
                ...
                if (latch == null) {
                    Log.e(MailChat.LOG_TAG, "SleepService No CountDownLatch available with id = " + id);
                } else {
                    if (MailChat.DEBUG)
                        Log.d(MailChat.LOG_TAG, "SleepService Counting down CountDownLatch with id = " + id);
                    latch.countDown();
                }
                ...
                if (MailChat.DEBUG)
                    Log.d(MailChat.LOG_TAG, "SleepService Sleep for id " + id + " already finished");
            }
        }
    }
    ...
    private static void reacquireWakeLock(SleepDatum sleepDatum) {
        ...
        if (wakeLock != null) {
            synchronized (wakeLock) {
                ...
                if (MailChat.DEBUG)
                    Log.d(MailChat.LOG_TAG, "SleepService Acquiring wakeLock for " + timeout + "ms");
