 ...
 * thread and task queuing for all MailChat services to use.
 * </p><p>
...
public abstract class CoreService extends Service {
    ...
    public static String WAKE_LOCK_ID = "cn.mailchat.service.CoreService.wakeLockId";
    ...
    protected static void addWakeLock(Context context, Intent intent) {
        TracingWakeLock wakeLock = acquireWakeLock(context, "CoreService addWakeLock",
                MailChat.MAIL_SERVICE_WAKE_LOCK_TIMEOUT);
    ...
    public void onCreate() {
        if (MailChat.DEBUG) {
            Log.i(MailChat.LOG_TAG, "CoreService: " + className + ".onCreate()");
        }
    ...
    public final int onStartCommand(Intent intent, int flags, int startId) {
         ...
         * services are started from MailChat.onCreate() when the Application object is initialized.
         *
         * See issue 3750
         */
        if (intent == null) {
        ...
        if (MailChat.DEBUG) {
            Log.i(MailChat.LOG_TAG, "CoreService: " + className + ".onStart(" + intent + ", " + startId + ")");
        }
        ...
        if (coreWakeLockId != -1) {
            if (MailChat.DEBUG) {
                Log.d(MailChat.LOG_TAG, "Got core wake lock id " + coreWakeLockId);
            }
            ...
            if (coreWakeLock != null) {
                if (MailChat.DEBUG) {
                    Log.d(MailChat.LOG_TAG, "Found core wake lock with id " + coreWakeLockId +
                            ", releasing");
                }
    ...
    public void execute(Context context, final Runnable runner, int wakeLockTime,
        ...
        Runnable myRunner = new Runnable() {
            public void run() {
                try {
                    ...
                    if (MailChat.DEBUG) {
                        Log.d(MailChat.LOG_TAG, "CoreService (" + className + ") running Runnable " +
                    ...
                    try {
                        if (MailChat.DEBUG) {
                            Log.d(MailChat.LOG_TAG, "CoreService (" + className + ") completed " +
        ...
        if (mThreadPool == null) {
            Log.e(MailChat.LOG_TAG, "CoreService.execute (" + className + ") called with no thread " +
            ...
            if (MailChat.DEBUG) {
                Log.d(MailChat.LOG_TAG, "CoreService (" + className + ") queueing Runnable " +
                        runner.hashCode() + " with startId " + startId);
            }
            ...
            try {
                ...
                Log.i(MailChat.LOG_TAG, "CoreService: " + className + " is shutting down, ignoring " +
    ...
    public void onLowMemory() {
        Log.w(MailChat.LOG_TAG, "CoreService: " + className + ".onLowMemory() - Running low on memory");
    }
    ...
    public void onDestroy() {
        if (MailChat.DEBUG) {
            Log.i(MailChat.LOG_TAG, "CoreService: " + className + ".onDestroy()");
        }
