 ...
 * 
 * RFC822 email address may have following format. "name" <address> (comment) "name" <address> name <address>
 * address Name and comment part should be MIME/base64 encoded in header if necessary.
 * 
 */
public class Address {
	...
	public void setPersonal(String personal) {
		if (personal != null) {
			...
			personal = DecoderUtil.decodeEncodedWords(personal, DecodeMonitor.SILENT);
...
//	}
	 ...
	 * @return
	 */
	public static boolean isAllValid(String addressList) {// 验证邮箱的正则表达式
		String format = "[a-zA-Z0-9_\\-\\.]+@[a-zA-Z0-9_\\-\\.]+(\\.(com|cn|org|edu|hk))";
		if (addressList != null && addressList.length() > 0) {
			Rfc822Token[] tokens = Rfc822Tokenizer.tokenize(addressList);
			for (int i = 0, length = tokens.length; i < length; ++i) {
				Rfc822Token token = tokens[i];
				String address = token.getAddress();
				if (!TextUtils.isEmpty(address) && !address.matches(format)) {
	 ...
	 * @date:2013-8-13
	 */
	public static ArrayList<String> getAddressList(Address[] addresses) {
	 ...
	 * 
	 * @return RFC822/MIME compatible address string. It may be surrounded by double quote or quoted and
	 *         MIME/base64 encoded if necessary.
	 */
	 ...
	 *         quoted and MIME/base64 encoded if necessary.
	 */
	...
	/**
	 * Convert a packed list of addresses to a form suitable for use in an RFC822 header. This implementation
	 * is brute-force, and could be replaced with a more efficient version if desired.
	 */
	...
	/**
	 * Unpacks an address list that is either CSV of RFC822 addresses OR (for backward compatibility)
	 * previously packed with pack()
	 * 
	 * @param addressList
	 *            string packed with pack() or CSV of RFC822 addresses
	...
	public static Address[] unpack(String addressList) {
		...
		if ((addressList.indexOf(LIST_DELIMITER_PERSONAL) == -1) && (addressList.indexOf(LIST_DELIMITER_EMAIL) == -1)) {
	...
	/**
	 * Generate a String containing RFC822 addresses separated by commas NOTE: We used to "pack" these
	 * addresses in an app-specific format, but no longer do so
	 */
	...
	public String toEncodedString() {
		if (!StringUtil.isEmpty(mPersonal)) {
			return EncoderUtil.encodeAddressDisplayName(mPersonal) + " <" + mAddress + ">";
		} else {
			return mAddress;
		}
