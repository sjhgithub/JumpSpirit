...
//		bitmap = Bitmap.createScaledBitmap(bitmap, GlobalTools.dip2px(context, 24), GlobalTools.dip2px(context, 24), true);
...
//				bitmap = Bitmap.createScaledBitmap(bitmap, GlobalTools.dip2px(context, 24), GlobalTools.dip2px(context, 24), true);
	...
	private List<ChatEmoji> getData(int page) {
		...
		if (list.size() == pageSize) {
			ChatEmoji object = new ChatEmoji();
			object.setId(R.drawable.selector_face_del);
			list.add(object);
		}
		return list;
	}
