package cn.mailchat.helper.power;
...
public class TracingPowerManager {
    ...
    public static synchronized TracingPowerManager getPowerManager(Context context) {
        ...
        if (tracingPowerManager == null) {
            if (MailChat.DEBUG) {
                Log.v(MailChat.LOG_TAG, "Creating TracingPowerManager");
            }
    ...
    public class TracingWakeLock {
        ...
        public TracingWakeLock(int flags, String ntag) {
            ...
            if (MailChat.DEBUG) {
                Log.v(MailChat.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ": Create");
            }
        }
        public void acquire(long timeout) {
            ...
            if (MailChat.DEBUG) {
                Log.v(MailChat.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + " for " + timeout + " ms: acquired");
            }
            raiseNotification();
            if (startTime == null) {
        ...
        public void acquire() {
            ...
            raiseNotification();
            if (MailChat.DEBUG) {
                Log.w(MailChat.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ": acquired with no timeout.  K-9 Mail should not do this");
            }
            if (startTime == null) {
                startTime = System.currentTimeMillis();
            }
        ...
        public void release() {
            if (startTime != null) {
                ...
                if (MailChat.DEBUG) {
                    Log.v(MailChat.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ": releasing after " + (endTime - startTime) + " ms, timeout = " + timeout + " ms");
                }
            } else {
                if (MailChat.DEBUG) {
                    Log.v(MailChat.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ", timeout = " + timeout + " ms: releasing");
                }
            }
            cancelNotification();
            synchronized (wakeLock) {
                ...
                synchronized (timer) {
                    ...
                    timerTask = new TimerTask() {
                        ...
                        public void run() {
                            if (startTime != null) {
                                Long endTime = System.currentTimeMillis();
                                Log.i(MailChat.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ": has been active for "
                                      + (endTime - startTime) + " ms, timeout = " + timeout + " ms");

                            } else {
                                Log.i(MailChat.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ": still active, timeout = " + timeout + " ms");
                            }
                        }
