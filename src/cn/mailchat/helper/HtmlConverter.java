...
public class HtmlConverter {
    ...
    private static class HtmlToTextTagHandler implements Html.TagHandler {
        ...
        /**
         * Fetch the matching opening Annotation object and verify that it's the one added by MailChat.
     ...
     * {@link cn.mailchat.view#MessageWebView.setText(String) MessageWebView.setText()}
     * </p>
     *
     * @param text
     *         Plain text string.
    ...
    private static String simpleTextToHtml(String text) {
        ...
        try {
            ...
            //Should never happen
            Log.e(MailChat.LOG_TAG, "Could not read string to convert text to HTML:", e);
        }
    ...
    protected static void linkifyText(final String text, final StringBuffer outputBuffer) {
        ...
        // Modified by LL
        // BEGIN
        String sig = MailChat.getInstance().getString(R.string.default_signature);
        if (sig != null) {
            sig = sig.replaceAll(" ", "&nbsp;");
            int index = outputBuffer.lastIndexOf(sig);
            while (index != -1) {
            	outputBuffer.delete(index, index + sig.length());
            	outputBuffer.insert(index, MailChat.getInstance().getString(R.string.default_signature_html));
            	index = outputBuffer.lastIndexOf(sig);
            }
        }
        // END
    }
    ...
    public static String cssStylePre() {
        final String font = MailChat.messageViewFixedWidthFont()
                ? "monospace"
                : "sans-serif";
    ...
    public static String textToHtmlFragment(final String text) {
        ...
        if (!StringUtils.isNullOrEmpty(htmlified)) {
            if (htmlified.contains(" ")) {
                htmlified = htmlified.replaceAll(" ", "&nbsp;");
            }
        }
