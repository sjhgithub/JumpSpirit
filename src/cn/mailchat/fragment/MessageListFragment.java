...
import cn.mailchat.activity.misc.ContactPictureLoader;
...
import cn.mailchat.helper.ContactPicture;
import cn.mailchat.helper.MergeCursorWithUniqueId;
...
import cn.mailchat.mail.store.LocalStore.AttachmentInfo;
...
import cn.mailchat.provider.EmailProvider.SpecialColumns;
import cn.mailchat.provider.EmailProvider.ThreadColumns;
...
import cn.mailchat.view.AttachmentSquareView;
import cn.mailchat.view.AttachmentView;
import cn.mailchat.view.MailChatPullToRefreshListView;
import cn.mailchat.view.MailChatPullToRefreshListView.PullToRefreshListener;
...
public class MessageListFragment extends Fragment implements
	...
	private static final String[] PROJECTION = Arrays.copyOf(THREADED_PROJECTION,
	        THREAD_COUNT_COLUMN);
	// 控制长按删除只toast一次
	private boolean isShowDeleteToast = false;
	private String folderSecName="";//邮件夹二级标题，比如收藏什么的
	private boolean isRefresh=false;
	private boolean isAutoRefresh=false;//表示自动刷新
	...
	private MailChatPullToRefreshListView mPullToRefreshView;
	...
	private String pull_to_refresh_start_pull_label;
	private String pull_to_refresh_start_release_label;
	private String pull_to_refresh_start_refreshing_label;
	private String pull_to_refresh_end_pull_label;
	private String pull_to_refresh_end_release_label;
	private String pull_to_refresh_end_refreshing_label;

//	private View mSearchView;
	private View mHeaderView;

	private boolean isRestoreSyncState;
	...
	public AtomicBoolean mIsLoadOngoing = new AtomicBoolean(false);
	private boolean mRemoteSearchPerformed = false;
	private Future<?> mRemoteSearchFuture = null;
	public List<SearchResult> mExtraSearchResults = new ArrayList<SearchResult>();
	...
	private TextView emptyMailList;
	...
	static class MessageListHandler extends Handler {
		...
		private static final int ACTION_COPY_OR_MOVE_TOAST = 8;
		private WeakReference<MessageListFragment> mFragment;

		// Show/Hide/Goto/Add header/footer view in UI thread
		// Modified by LL
		// BEGIN
		public void enablePullToRefresh(final MessageListFragment mlf, final boolean enable) {
			post(new Runnable() {
				@Override
				public void run() {
					mlf.setPullToRefreshEnabled(enable);
				}
			});
		}

		public void showHeader(final ListView lv, final View header) {
			post(new Runnable() {
				@Override
				public void run() {
		...
		public void hideHeader(final ListView lv, final View header) {
			post(new Runnable() {
				@Override
				public void run() {
					lv.removeHeaderView(header);
		...
		public void showFooter(final ListView lv, final View footer, final boolean reposition) {
			post(new Runnable() {
				@Override
				public void run() {
		...
		public void hideFooter(final ListView lv, final View footer) {
			post(new Runnable() {
				@Override
				public void run() {
					lv.removeFooterView(footer);
		...
		public void gotoFooter(final ListView lv) {
			post(new Runnable() {
				@Override
				public void run() {
					lv.setSelection(lv.getCount() - 1);
		...
		public void addFooter(final ListView lv, final View footer) {
			post(new Runnable() {
				@Override
				public void run() {
					if (lv.getFooterViewsCount() > 0) {
		...
		public void showReload(final MessageListFragment fragment, final String message) {
		    post(new Runnable() {
                @Override
                public void run() {
                    fragment.mReloadSubtitle.setText(message);
                    fragment.mReload.setVisibility(View.VISIBLE);
                }
            });
		}
		
		public void hideReload(final MessageListFragment fragment) {
            post(new Runnable() {
                @Override
                public void run() {
                    fragment.mReloadSubtitle.setText("");
                    fragment.mReload.setVisibility(View.GONE);
                }
            });
        }
		// END
		...
		// Modified by LL
		public void updateFooter(final String message, final boolean isLoading) {
			post(new Runnable() {
				@Override
				public void run() {
					...
					if (fragment != null) {
						...
						// Modified by LL
						fragment.updateFooter(message, isLoading);
					}
				}
			});
		}
		...
		public void updateFooterView() {
		    post(new Runnable() {
                ...
                public void run() {
                    MessageListFragment fragment = mFragment.get();
                    if (fragment != null) {
                        fragment.updateFooterView();
                    }
                }
            });
		}

	    public void setSearchResultText(final String text, final boolean isLoading) {
            post(new Runnable() {
                @Override
                public void run() {
                    MessageListFragment fragment = mFragment.get();
                    if (fragment != null) {
                        fragment.searchResult.setText(text);
                        if (isLoading) {
                            fragment.searchIndicator.setVisibility(View.VISIBLE);
                        } else {
                            fragment.searchIndicator.setVisibility(View.GONE);
                        }
                    }
                }
            });
		...
		public void copyOrMoveToast(boolean isCopy, boolean isSucc) {
			android.os.Message msg = android.os.Message.obtain(this,
					ACTION_COPY_OR_MOVE_TOAST, (isSucc) ? 1 : 0, 0, isCopy);
			sendMessage(msg);
		}
		...
		public void handleMessage(android.os.Message msg) {
			...
			case ACTION_COPY_OR_MOVE_TOAST: {
				boolean isSucc = (msg.arg1 == 1);
				boolean isCopy = (boolean) msg.obj;
				if (isCopy) {
					if (isSucc) {
					...
					if (isSucc) {
						Toast.makeText(activity,
								activity.getString(R.string.move_success),
								Toast.LENGTH_SHORT).show();
					} else {
						Toast.makeText(activity,
	...
	private void setWindowTitle() {
		...
		if (!isManualSearch() && mSingleFolderMode) {
			String displayName = FolderInfoHolder.getDisplayName(getActivity(),
					mAccount, mFolderName);
			mFragmentListener.setMessageListTitle(displayName+folderSecName);
		}
		// set unread count
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		if (view == mFooterView) {
			 ...
			 * 
			 * int numResults = mExtraSearchResults.size(); int limit =
			 * mAccount.getRemoteSearchNumResults();
			 * 
			 * List<Message> toProcess = mExtraSearchResults;
			 * 
			 * if (limit > 0 && numResults > limit) { toProcess =
			 * toProcess.subList(0, limit); mExtraSearchResults =
			 * mExtraSearchResults.subList(limit, mExtraSearchResults.size()); }
			 * else { mExtraSearchResults = null; //updateFooter(""); //
			 * Modified by LL updateFooter("", false); }
			 * 
		...
		/*
		 * Cursor cursor = (Cursor) parent.getItemAtPosition(position); if
		 * (cursor == null) { return; } if (mListView.getHeaderViewsCount()>0) {
		 * position=position-1; } if (mSelectedCount > 0) {
		 * toggleMessageSelect(position); } else { if (mThreadedList &&
		 * cursor.getInt(THREAD_COUNT_COLUMN) > 1) { Account account =
		 * getAccountFromCursor(cursor); long folderId =
		 * cursor.getLong(FOLDER_ID_COLUMN); String folderName =
		 * getFolderNameById(account, folderId);
		 * 
		 * // If threading is enabled and this item represents a thread, //
		 * display the thread contents. long rootId =
		 * cursor.getLong(THREAD_ROOT_COLUMN);
		 * mFragmentListener.showThread(account, folderName, rootId); } else {
		...
		synchronized (refreshingList) {
			// Disable message detail view during refreshing temporarily
			// Modified by LL enabled!!!
			/*
			 * if (isRefreshing) { Toast.makeText(mContext,
			 * R.string.message_list_refresh_ongoing,
			...
			position = position - mListView.getHeaderViewsCount() + 1;
			...
			if (mSelectedCount > 0) {
				...
				if (mThreadedList && cursor.getInt(THREAD_COUNT_COLUMN) > 1) {
					...
					// If threading is enabled and this item represents a
					// thread,
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mController = MessagingController.getInstance(getActivity()
				.getApplication());
		mPreviewLines = MailChat.messageListPreviewLines();
		// 多选
		// mCheckboxes = MailChat.messageListCheckboxes();
		mStars = MailChat.messageListStars();

		if (MailChat.showContactPicture()) {
	...
	// Automatic refresh folder after switching.
	// Modified by LL
	// BEGIN

	// 正在刷新的文件夹列表
	private static class RefreshingList {
		private HashMap<String, Boolean> refreshingList = new HashMap<String, Boolean>();

		private void addRefreshing(String account, String folder) {
		    refreshingList.put(account + folder, true);
		}

		private void removeRefreshing(String account, String folder) {
		    refreshingList.remove(account + folder);
		}

		private boolean isRefreshing(String account, String folder) {
		    return refreshingList.containsKey(account + folder);
		}

		private boolean isRefreshing() {
			return !refreshingList.isEmpty();
		}
	}

	private static RefreshingList refreshingList = new RefreshingList();
	...
	public void onStart() {
		...
		if (mSearch.isManualSearch()) {
		    mHandler.hideHeader(mListView, getHeaderView(mListView));
		    if (mCurrentFolder != null && isRemoteSearch()
	                && mExtraSearchResults != null
	                && mExtraSearchResults.size() > 0) {
		        mHandler.showFooter(mListView, getFooterView(mListView), false);
		    } else if (MailChat.remoteSearchPaging) {
		        mHandler.showFooter(mListView, getFooterView(mListView), false);
		    } else if (!mIsLoadOngoing.get()) {
		        mHandler.hideFooter(mListView, getFooterView(mListView));
		    }
            mHandler.enablePullToRefresh(this, false);
		    MailChat.forceRefresh = false;
		    return;
		}
		...
		synchronized (refreshingList) {
			...
			if (refreshingList.isRefreshing()) {
				mHandler.progress(true);

				if (refreshingList.isRefreshing(mAccount.getUuid(), mFolderName)) {
					mHandler.showHeader(mListView, getHeaderView(mListView));
					mHandler.hideFooter(mListView, getFooterView(mListView));
			...
			if (!isRefreshing && mFolderName != null
					&& !mFolderName.equals(mAccount.getOutboxFolderName())
					//&& !mFolderName.equals(mAccount.getSentFolderName())
					&& !mFolderName.equals(mAccount.getDraftsFolderName())
					&& !mFolderName.equals(mAccount.getTrashFolderName())
					&& !mFolderName.equals(mAccount.getSpamFolderName())
					&& !mFolderName.equals(mAccount.getErrorFolderName())) {
				if (MailChat.forceRefresh) {
					checkMail();
					isAutoRefresh=true;
				} else {
					long lastUpdate = 0;
					try {
						lastUpdate = mAccount.getRemoteStore().getFolder(mFolderName).getLastUpdate();
	...
	// 必须在UI线程中调用
	public void setActionBarProgress(Main4TabActivity activity) {
		synchronized (refreshingList) {
			if (refreshingList.isRefreshing()) {
				activity.enableActionBarProgress(true);
			}
		}
	}
    ...
    public void onDetach() {
	    synchronized (MailChat.remoteSearchList) {
            try {
                ...
                LocalFolder localFolder = localStore.getFolder(mFolderName);
                for (SearchResult result : MailChat.remoteSearchList) {
                    Message message = localFolder.getMessage(result.getUid());
                    if (message != null) {
                        if (SearchResult.Type.LOCAL.equals(result.getType())) {
            ...
            MailChat.remoteSearchList.clear();
	    }
        super.onDetach();
    }
	...
	private void initializeSortSettings() {
		if (mSingleAccountMode) {
			...
			mSortType = MailChat.getSortType();
			mSortAscending = MailChat.isSortAscending(mSortType);
			mSortDateAscending = MailChat.isSortAscending(SortType.SORT_DATE);
		}
	}
	...
	private void decodeArguments() {
		...
		mThreadedList = false;
		mIsThreadDisplay = false;
		mSearch = args.getParcelable(ARG_SEARCH);
		mTitle = mSearch.getName();
		folderSecName=mSearch.getFolderSecName();
	...
	private void initializeMessageList() {
		 ...
		 * updateFooterView(); }
		 */
		// Trick to enable header/footer view before setting the adapter
		// Modified by LL
		// BEGIN
		synchronized (refreshingList) {
			mListView.addHeaderView(getHeaderView(mListView));
			mListView.removeHeaderView(getHeaderView(mListView));
			mListView.addFooterView(getFooterView(mListView));

			// Hide footer view on local folder to disable refresh action
			// Modified by LL
			// BEGIN
			if (!isRemoteSearchAllowed() && !isCheckMailSupported()) {
				mListView.removeFooterView(getFooterView(mListView));
			}
			// END
	...
	private FolderInfoHolder getFolder(String folder, Account account) {
		...
		try {
			...
			Log.e(MailChat.LOG_TAG, "getFolder(" + folder + ") goes boom: ", e);
			return null;
		} finally {
			if (localFolder != null) {
				localFolder.close();
	...
	private String getFolderNameById(Account account, long folderId) {
		try {
			...
			Log.e(MailChat.LOG_TAG, "getFolderNameById() failed.", e);
		}
	...
	public void onResume() {
		...
		mSenderAboveSubject = MailChat.messageListSenderAboveSubject();
		...
		isRestoreSyncState = true;
		mController.addListener(mListener);
		isRestoreSyncState = false;
	...
	private void initializePullToRefresh(LayoutInflater inflater, View layout) {
		layoutNoData = (LinearLayout) layout.findViewById(R.id.chat_list_watermark);
		loadingView = (LinearLayout) layout.findViewById(R.id.loadingLayout);
		btnRefresh = (TextView) layout.findViewById(R.id.no_chat);
		btnRefresh.setText(getString(R.string.refresh));
		emptyMailList = (TextView) layout.findViewById(R.id.empty_mail_list);
		ImageView imgBg = (ImageView) layout.findViewById(R.id.no_data_bg);
		imgBg.setBackgroundResource(R.drawable.email_no_data);
		mPullToRefreshView = (MailChatPullToRefreshListView) layout
				.findViewById(R.id.message_list);
		searchResult = (TextView) layout.findViewById(R.id.search_result_no_data);
		searchIndicator = (ProgressBar) layout.findViewById(R.id.search_ongoing_indicator);
		// Set empty view
		// loadingView = inflater
		...
		btnRefresh.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				dosynchronizeMailbox();
			    isAutoRefresh=false;
			}

		});
		emptyMailList.setOnClickListener(new OnClickListener() {

            @Override
            public void onClick(View v) {
                WebViewWithErrorViewActivity.forwardOpenUrlActivity(
                        getActivity(),
                        "https://a.mailchat.cn/static/mcpage/EmptyMailList.html",
                        null,
                        Preferences.getPreferences(mContext).getDefaultAccount().getUuid(),
                        -1,
                        false);
            }
		...
		/*
		 * if (isRemoteSearchAllowed()) { // "Pull to search server"
		 * mPullToRefreshView .setOnRefreshListener(new
		 * PullToRefreshBase.OnRefreshListener<ListView>() {
		 * 
		 * @Override public void onRefresh( PullToRefreshBase<ListView>
		 * refreshView) { mPullToRefreshView.onRefreshComplete();
		 * onRemoteSearchRequested(); } }); ILoadingLayout proxy =
		 ...
		 * proxy.setReleaseLabel(getString(R.string.
		 * pull_to_refresh_remote_search_from_local_search_release)); } else if
		 * (isCheckMailSupported()) { // "Pull to refresh" mPullToRefreshView
		 * .setOnRefreshListener(new
		 * PullToRefreshBase.OnRefreshListener<ListView>() {
		 * 
		 * @Override public void onRefresh( PullToRefreshBase<ListView>
		 * refreshView) { checkMail(); } }); }
		 */
		// Use OnRefreshListener2 to support both sides' pull-to-refresh
		// Modified by LL
		// BEGIN
		...
		mPullToRefreshView
				.setPullToRefreshListener(new PullToRefreshListener() {
					...
					@Override
					public void onPullToRefresh(Mode mode) {
						synchronized (refreshingList) {
							if (!refreshingList.isRefreshing(mAccount.getUuid(),
							        mFolderName)) {
								mHandler.hideFooter(mListView, getFooterView(mListView));
							}
						}
					...
					@Override
					public void onReleaseToRefresh(Mode mode) {
					...
					@Override
					public void onReset(Mode mode) {
						synchronized (refreshingList) {
							if (!refreshingList.isRefreshing(mAccount.getUuid(),
							        mFolderName)) {
								if (mPullToRefreshView.getCurrentMode() == Mode.PULL_FROM_END) {
									if (mListView.getFooterViewsCount() == 0) {
										mHandler.showFooter(mListView,
												getFooterView(mListView),
												true);
									}
								} else {
									mHandler.addFooter(mListView,
											getFooterView(mListView));
								}
							}
						}
		...
		mPullToRefreshView.setShowViewWhileRefreshing(false);

		Resources r = getActivity().getResources();
		pull_to_refresh_start_pull_label = r
				.getString(R.string.pull_to_refresh_start_pull_label);
		pull_to_refresh_start_release_label = r
				.getString(R.string.pull_to_refresh_start_release_label);
		pull_to_refresh_start_refreshing_label = r
				.getString(R.string.pull_to_refresh_start_refreshing_label);
		pull_to_refresh_end_pull_label = r
				.getString(R.string.pull_to_refresh_end_pull_label);
		pull_to_refresh_end_release_label = r
				.getString(R.string.pull_to_refresh_end_release_label);
		pull_to_refresh_end_refreshing_label = r
				.getString(R.string.pull_to_refresh_end_refreshing_label);
		// END

		// Disable pull-to-refresh until the message list has been loaded
		// setPullToRefreshEnabled(false);
		
		mReload = (LinearLayout) layout.findViewById(R.id.message_list_reload);
		mReloadTitle = (TextView) mReload.findViewById(R.id.message_list_reload_title);
		mReloadSubtitle = (TextView) mReload.findViewById(R.id.message_list_reload_subtitle);
		mReloadButton = (TextView) mReload.findViewById(R.id.message_list_reload_button);
    ...
    private void dosynchronizeMailbox() {
        // Show header view and hide footer view during refresh
        // Modified by LL
        // BEGIN
        synchronized (refreshingList) {
            if (refreshingList.isRefreshing(mAccount.getUuid(), mFolderName)) {
                Toast.makeText(mContext, R.string.message_list_refresh_ongoing,
                        Toast.LENGTH_SHORT).show();
                return;
            } else {
                mHandler.showHeader(mListView, getHeaderView(mListView));
                mHandler.hideFooter(mListView, getFooterView(mListView));
                refreshingList.addRefreshing(mAccount.getUuid(), mFolderName);
                setPullToRefreshEnabled(false);
            }
        }
        // END
	...
	private void setPullToRefreshEnabled(boolean enable) {
		 ...
		 */
		// Set mode to support both sides' pull-to-refresh and update hint
		// labels
		// Modified by LL
		// BEGIN

		mPullToRefreshView.setMode((enable) ? PullToRefreshBase.Mode.BOTH
				: PullToRefreshBase.Mode.DISABLED);

		ILoadingLayout startProxy = mPullToRefreshView.getLoadingLayoutProxy(
				true, false);
		ILoadingLayout endProxy = mPullToRefreshView.getLoadingLayoutProxy(
				false, true);

		startProxy.setPullLabel(pull_to_refresh_start_pull_label);
		startProxy.setReleaseLabel(pull_to_refresh_start_release_label);
		startProxy.setRefreshingLabel(pull_to_refresh_start_refreshing_label);

		endProxy.setPullLabel(pull_to_refresh_end_pull_label);
		endProxy.setReleaseLabel(pull_to_refresh_end_release_label);
		endProxy.setRefreshingLabel(pull_to_refresh_end_refreshing_label);
		// END
	}
	...
	public void onForward(Message message) {
		if (checkForwardPossible(message)) {
	...
	private void changeSort(SortType sortType, Boolean sortAscending) {
		...
		if (account != null) {
			...
			MailChat.setSortType(mSortType);
			...
			MailChat.setSortAscending(mSortType, mSortAscending);
			mSortDateAscending = MailChat.isSortAscending(SortType.SORT_DATE);
	...
	private void onDelete(List<Message> messages) {
		if (mAccount.isOnPushSuccess()) {
			mController.cancelDeleteMessageNotify(mAccount, messages);
		} else {

		}
		if (MailChat.confirmDelete()) {
	...
	private void onDeleteConfirmed(List<Message> messages) {
		if (mThreadedList) {
			...
			mController.deleteMessages(messages, true, null);
		}
	}
...
//		mController.sendPendingMessages(mAccount, null);
	...
	public boolean onContextItemSelected(android.view.MenuItem item) {
		...
		case R.id.move: {
			Message message = getMessageAtPosition(adapterPosition);
			onMove(message);
			MobclickAgent.onEvent(mContext, "move_mail");
			break;
		}
		case R.id.deselect:
			mAdapter.toggle(false);
			mFragmentListener.isHideHomeBottomBar(false);
			toggleMessageSelectWithAdapterPosition(adapterPosition);
			break;
		case R.id.select: {
			mAdapter.toggle(true);
			mFragmentListener.isHideHomeBottomBar(true);
			toggleMessageSelectWithAdapterPosition(adapterPosition);
			break;
		}
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		String draft = cursor.getString(FOLDER_NAME_COLUMN);
		menu.setHeaderTitle(subject);
		// 草稿箱的邮件长按操作菜单选项只有删除和多选。
		if (draft.equals(mAccount.getDraftsFolderName())) {
	...
	class MessageListActivityListener extends ActivityListener {
		@Override
		public void messageDeleted(final Account account, final String folder,
				Message message) {
			if (account.getEmail().equals(mAccount.getEmail())) {
				mHandler.post(new Runnable() {
					...
					public void run() {
						...
						if (activity != null
								&& folder != account.getDraftsFolderName()
								&& !isShowDeleteToast) {
							isShowDeleteToast = true;
							Toast.makeText(activity, R.string.delete_success,
									Toast.LENGTH_SHORT).show();
						}
					}
				});
		...
		public void remoteSearchFailed(Account acct, String folder,
				final String err) {
		    MailChat.toast(getString(R.string.remote_search_error));
		}

        @Override
        public void enableProgressIndicator(boolean enable) {
            mIsLoadOngoing.set(enable);
            if (!mIsLoadOngoing.get()) {
                mHandler.updateFooterView();
            }
		...
		public void remoteSearchStarted(Account acct, String folder) {
			 ...
			 */
			// Modified by LL
			String text = getString(R.string.remote_search_sending_query);
			mHandler.updateFooter(text, true);
			mHandler.setSearchResultText(text, true);
			MailChat.toast(text);
		}
		...
		public void remoteSearchFinished(Account acct, String folder,
		    ...
		    mIsLoadOngoing.set(false);
			mHandler.progress(false);
			mHandler.remoteSearchFinished();
			mHandler.setSearchResultText(getString(R.string.search_no_result), false);
			
			if (extraResults != null && extraResults.size() > 0) {
			    for (Message msg : extraResults) {
			        mExtraSearchResults.add(new SearchResult(msg.getUid(), SearchResult.Type.REMOTE));
			    }
				/*
				 ...
				 */
				// Modified by LL
				mHandler.updateFooter(mContext.getString(R.string.load_more_remote_search_result), false);
			} else if (MailChat.remoteSearchPaging) {
			    mHandler.updateFooter(mContext.getString(R.string.load_more_remote_search_result), false);
			} else {
			...
			//MailChat.toast(getString(R.string.remote_search_finished));
		}
        ...
        public void remoteSearchAddMessage(Account account, String folder,
		    ...
		    synchronized (MailChat.remoteSearchList) {
		        MailChat.remoteSearchList.add(new SearchResult(message.getUid(), SearchResult.Type.REMOTE));
    		    try {
    		        if (!message.isSet(Flag.X_REMOTE_SEARCH_MAIL)) {
    		            message.setFlag(Flag.X_REMOTE_SEARCH_MAIL, true);
    		        }
    		    } catch (Exception e) {
    		        // DO NOTHING
    		    }
		...
		public void remoteSearchServerQueryComplete(Account account,
			...
			if (account != null && account.getRemoteSearchNumResults() != 0
				 ...
				 */
				// Modified by LL
			    text = getString(R.string.remote_search_downloading_limited,
				 ...
				 */
				// Modified by LL
			    text = getString(R.string.remote_search_downloading, numResults);
				mHandler.updateFooter(text, true);
			}
		...
		public void synchronizeMailboxStarted(Account account, String folder) {
		    mHandler.hideReload(MessageListFragment.this);
		...
		public void synchronizeMailboxFinished(Account account, String folder,
				final int totalMessagesInMailbox, int numNewMessages) {
		    /*
//          if (isStopProgress) {
            if (isAdded()) {
    			getActivity().runOnUiThread(new Runnable() {
    				public void run() {
    					if (totalMessagesInMailbox > 0 && !isRefresh) {
    						layoutNoData.setVisibility(View.GONE);
    					} else {
			...
			*/
            ...
            try {
                if (updateForMe(account, folder)) {
                    ...
                    messageCount = localFolder.getMessageCount();
                }
            } catch (MessagingException e) {
                // DO NOTHING
            }
...
//			}
			/*
			 * if (updateForMe(account, folder)) { mHandler.progress(false);
			...
			synchronized (refreshingList) {
				if (updateForMe(account, folder)) {
					...
					mHandler.hideHeader(mListView, getHeaderView(mListView));
					mHandler.addFooter(mListView, getFooterView(mListView));
					
					if (!mSearch.isManualSearch()) {
					    mHandler.enablePullToRefresh(MessageListFragment.this, true);
					}
				}

				refreshingList.removeRefreshing(account.getUuid(), folder);
				if (!refreshingList.isRefreshing()) {
					mHandler.progress(false);
				}
			}
		...
		public void synchronizeMailboxFailed(Account account, String folder, String message) {
			if (MailChat.isNotifySyncError()
			        && !isRestoreSyncState
			        && mAccount.getEmail().equals(account.getEmail())) {
			    mHandler.showReload(MessageListFragment.this, message);
			}
		...
		public void copyOrMoveMailSuccess(Account account, boolean isCopy) {
			mHandler.copyOrMoveToast(isCopy, true);
		}

		@Override
		public void copyOrMoveFailed(Account account, boolean isCopy) {
			mHandler.copyOrMoveToast(isCopy, false);
		}
		@Override
		public void getContactForMessageSuccess(Account acc, String email,
				ContactAttribute contact) {
			if (acc.getUuid().equals(mAccount.getUuid())) {
				ContactInfoActivity
						.actionView(mContext, mAccount, contact);
			}
		}
	}
	 ...
	 * 
	 * @copyright © 35.com
	 * @file name ：MessageListFragment.java
	 * @author ：zhangjx
	 * @create Data ：2014-10-28下午5:34:03
	 * @Current Version：v1.0
	 * @History memory :
	 * @Date : 2014-10-28下午5:34:03
	 * @Modified by：zhangjx
	 * @Description :邮件列表适配器
	 */
	class MessageListAdapter extends CursorAdapter {
		...
		MessageListAdapter() {
			...
			mFavoriteIconOn =  getResources().getDrawable(R.drawable.message_view_bottom_collect_small_p);
			mFavoriteIconOff =  getResources().getDrawable(R.drawable.message_view_bottom_collect_small_n);
		...
		public View newView(Context context, Cursor cursor, ViewGroup parent) {
			...
			holder.mailContent = (TextView) view
					.findViewById(R.id.mail_content);
			holder.unreadFlag = (ImageView) view
					.findViewById(R.id.img_unread_flag);
			...
			holder.flagged = (ImageView) view.findViewById(R.id.flagged);
			...
			holder.atts = new AttachmentSquareView[3];
			holder.atts[0] = (AttachmentSquareView)view.findViewById(R.id.att_content_1);
			holder.atts[1] = (AttachmentSquareView)view.findViewById(R.id.att_content_2);
			holder.atts[2] = (AttachmentSquareView)view.findViewById(R.id.att_content_3);
			holder.atts[0].setInflater(mInflater);
			holder.atts[1].setInflater(mInflater);
			holder.atts[2].setInflater(mInflater);
			holder.attCount = (TextView)view.findViewById(R.id.att_count);
			holder.dateSeperator = (ViewGroup) view.findViewById(R.id.date_seperator);
			holder.dateSeperator.setOnClickListener(holder);
			holder.dateSeperatorDate = (TextView) view.findViewById(R.id.date_seperator_date);
			holder.dateSeperatorDayOfWeek = (TextView) view.findViewById(R.id.date_seperator_day_of_week);
		...
		public void bindView(View view, Context context, Cursor cursor) {
			...
			// 收件时间
			Date date = new Date(cursor.getLong(DATE_COLUMN));
			String displayTime = DateFormat.is24HourFormat(mContext)
			        ? TimeUtils.DateFormatHM.format(date)
			        : TimeUtils.DateFormatHHMM.format(date);
			String displayDate = TimeUtils.DataFormatENYYMMDD.format(date);
			...
			int attachmentCount = cursor.getInt(ATTACHMENT_COUNT_COLUMN);
			...
			holder.selected.setVisibility((mCheckboxes) ? View.VISIBLE
					: View.GONE);
			if (mCheckboxes) {
...
//				holder.flagged.setChecked(flagged);
				holder.flagged.setImageDrawable(flagged?mFavoriteIconOn:mFavoriteIconOff);
			}
			...
			// Background color
			// if (selected || MailChat.useBackgroundAsUnreadIndicator()) {
			// int res;
			// if (selected) {
			...
			// modify by zhyq
			if (read) {
				// holder.preview.setTextColor(getResources().getColor(R.color.red));
				// holder.subject.setTextColor(getResources().getColor(R.color.red));
				// holder.preview.setTextColor(getResources().getColor(
				// R.color.mark_read));
				// holder.subject.setTextColor(getResources().getColor(
				// R.color.mark_read));
				holder.preview.getPaint().setFakeBoldText(false);
				holder.subject.getPaint().setFakeBoldText(false);
				holder.unreadFlag.setVisibility(View.INVISIBLE);
			} else {
				// holder.preview.setTextColor(getResources().getColor(
				// R.color.unread_preview));
				// holder.subject.setTextColor(getResources().getColor(
				// R.color.unread_subject));
				holder.preview.getPaint().setFakeBoldText(true);
			...
			// Log.d(TAG, "---->2040---->messageStringBuilder:" +
			...
			String mailContent = "";
			String tempContent = cursor.getString(PREVIEW_COLUMN);
			if (tempContent != null) {
				mailContent = tempContent;
			}
			holder.mailContent.setText(mailContent,
					TextView.BufferType.SPANNABLE);
			// Log.e(TAG, "---->2061---->mailContent:" + mailContent);
			...
			// int color = (MailChat.getK9Theme() == MailChat.Theme.LIGHT) ?
			...
			if (holder.subject != null) {
				if (!mSenderAboveSubject) {
					holder.preview.setCompoundDrawablesWithIntrinsicBounds(
							null, // left
							null, // top
							hasAttachments ? mAttachmentIcon : null, // right
							null); // bottom
			...
			// 附件快捷操作区块
			for (View att : holder.atts) {
			    att.setVisibility(View.GONE);
			}
			holder.attCount.setVisibility(View.GONE);
			if (MailChat.isMessageListAttachmentShortcuts() && attachmentCount > 0) {
			    ...
			    try {
                    AttachmentInfo[] atts = account.getLocalStore().getAttachmentInfos(Integer.toString(id));
                    ...
                    for (int i = 0; i < atts.length; i++) {
                        if (atts[i] != null) {
                            ...
                            String filename = MimeUtility.decode(atts[i].name);
                            holder.atts[i].setType(MimeUtility.getExtension(filename));
                            holder.atts[i].setStatus(atts[i].uri != null
                                    ? getString(R.string.attachment_downloaded)
                                    : SizeFormatter.formatSize(context,
                                            MimeUtility.getDecodeBase64Size(atts[i].size)));
                            holder.atts[i].setTitle(filename);
                            holder.atts[i].setMessageReference(ref);
                            holder.atts[i].setStore(atts[i].store);
                            holder.atts[i].setUri(atts[i].uri);
                            holder.atts[i].setProgress(0);
                            
                            String downloadingId = atts[i].id + ref.uid + ref.accountUuid;
                            holder.atts[i].setDownloadingId(downloadingId);
                            synchronized(MailChat.downloadingList) {
                                if (MailChat.downloadingList.containsKey(downloadingId)) {
                                    AttachmentView av = MailChat.downloadingList.get(downloadingId);
                                    if (av.mDownloadingNotification != null) {
                                        av.mDownloadingNotification.mSquareView = holder.atts[i];
                                        holder.atts[i].setProgress(av.mDownloadingNotification.getProgress());
                                    }
                                }
                            }
                            
                            holder.atts[i].setVisibility(View.VISIBLE);
                        }
                    }
                    
                    if (count > 0 && attachmentCount > 3) {
                        holder.attCount.setText(getString(R.string.attachment_count_hint,
                                attachmentCount));
                        holder.attCount.setVisibility(View.VISIBLE);
                    }
                } catch (MessagingException e) {
                    // DO NOTHING;
                }
			...
			holder.date.setText(displayTime);
			
			boolean showDateSeperator = true;
			if (cursor.moveToPrevious()) {
				Date previousDate = new Date(cursor.getLong(DATE_COLUMN));
				if (displayDate.equals(TimeUtils.DataFormatENYYMMDD.format(previousDate))) {
					showDateSeperator = false;
				}
				
				if (!cursor.moveToNext()) {
					Log.e(MailChat.LOG_TAG, "Move cursor back failed.");
				}
			}
			
			if (showDateSeperator) {
				holder.line.setVisibility(View.GONE);
				holder.dateSeperator.setVisibility(View.VISIBLE);
				holder.dateSeperatorDate.setText(displayDate);
				String displayDayOfWeek = TimeUtils.DateFormatDayOfWeek.format(date);
				holder.dateSeperatorDayOfWeek.setText(displayDayOfWeek);
			} else {
				holder.line.setVisibility(View.VISIBLE);
				holder.dateSeperator.setVisibility(View.GONE);
			}
		}

		public void toggle(boolean isSelete) {
			mCheckboxes = isSelete;
	...
	class MessageViewHolder implements View.OnClickListener {
		...
		public ViewGroup dateSeperator;
		public TextView dateSeperatorDate;
		public TextView dateSeperatorDayOfWeek;
		public View line;

		public AttachmentSquareView[] atts;
	...
	private View getHeaderView(ViewGroup parent) {
		if (mHeaderView == null) {
			mHeaderView = mInflater.inflate(R.layout.message_list_item_header,
					parent, false);
		}
	...
	private View getFooterView(ViewGroup parent) {
		if (mFooterView == null) {
			...
			// Modified by LL
			holder.icon = mFooterView.findViewById(R.id.main_icon);
	...
	private void updateFooterView() {
		if (!mSearch.isManualSearch() && mCurrentFolder != null
		    ...
		    if (mCurrentFolder != null
                    ...
                    && mExtraSearchResults != null
                    && mExtraSearchResults.size() > 0) {
                updateFooter(mContext.getString(R.string.load_more_remote_search_result), false);
            } else if (MailChat.remoteSearchPaging) {
                updateFooter(mContext.getString(R.string.load_more_remote_search_result), false);
            } else if (!mIsLoadOngoing.get()) {
                updateFooter("", false);
	...
	// Modified by LL
	public void updateFooter(final String text, boolean isLoading) {
	...
	private void onArchive(final List<Message> messages) {
		...
		for (Entry<Account, List<Message>> entry : messagesByAccount.entrySet()) {
			...
			if (!MailChat.FOLDER_NONE.equals(archiveFolder)) {
	...
	private void onSpamConfirmed(List<Message> messages) {
		...
		for (Entry<Account, List<Message>> entry : messagesByAccount.entrySet()) {
			...
			if (!MailChat.FOLDER_NONE.equals(spamFolder)) {
	 ...
	 * @file name ：MessageListFragment.java
	 * @author ：zhangjx
	 * @create Data ：2014-10-22下午6:54:19
	 * @Current Version：v1.0
	 * @History memory :
	 * @Date : 2014-10-22下午6:54:19
	 * @Modified by：zhangjx
	 * @Description :长按回调
	 */
	class ActionModeCallback implements ActionMode.Callback {
		 ...
		 * @History memory：
		 * @Date：2014-10-29 下午3:02:07 @Modified by：zhangjx
		 * @Description：菜单销毁时调用
		 */
		@Override
		 ...
		 *      android.view.Menu) function@Description: TODO
		 * @History memory：
		 * @Date：2014-10-29 下午1:49:07 @Modified by：zhangjx
		 * @Description：选择菜单
		 */
		@Override
		...
		public void showMarkAsRead(boolean show) {
			if (mActionMode != null&&mMarkAsRead!=null&&mMarkAsUnread!=null) {
		...
		public void showFlag(boolean show) {
			if (mActionMode != null&&mFlag!=null&&mUnflag!=null) {
...
//			mController.sendPendingMessages(mAccount, mListener);
	...
	public interface MessageListFragmentListener {
		...
		void isHideHomeBottomBar(boolean b);

		void updateMailSearchReasut(int searchReasutCount);
	}
	...
	public boolean isErrorFolder() {
		return MailChat.ERROR_FOLDER_NAME.equals(mFolderName);
	}
	...
	public void onRemoteSearch() {
		// Modified by shengli
		// BEGIN
		//由于搜索的时候未执行onResume里的获取网络状态方法，先在这里判断
		if (mHasConnectivity == null) {
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle args) {
		String accountUuid = mAccount.getUuid();//mAccountUuids[id];
		Account account = mAccount;//mPreferences.getAccount(accountUuid);
		if (MailChat.DEBUG) {
			Log.d(MailChat.LOG_TAG,
					"=MAIL LIST IS ACCOUNT NULL?=>" + account.getEmail());
		}
...
//		loadingView.setVisibility(View.GONE);
		...
		if (isManualSearch()) {
			...
			mFragmentListener.updateMailSearchReasut(data.getCount());
		} else {
			if (data.getCount() > 0) {
				//再次加重，发现有未选择的，显示选择全部
				mActionModeCallback.showSelectAll(mSelected.size() != mAdapter
	...
	/**
	 * 
	 * method name: resetActionMode function @Description: TODO Parameters and
	 * return values description： field_name void return type
	 * 
	 * @History memory：
	 * @Date：2014-10-29 下午2:49:35 @Modified by：zhangjx
	 * @Description：Starts or finishes the action mode when necessary.
	 * 
	 */
	...
	private void loadMoreMail() {
		...
		// Show footer view before loading more mail
	    if (!mSearch.isManualSearch()) {
    		synchronized (refreshingList) {
    			if (refreshingList.isRefreshing(mAccount.getUuid(), mFolderName)) {
    				Toast.makeText(mContext, R.string.message_list_refresh_ongoing,
    				...
    				mHandler.showFooter(mListView, getFooterView(mListView), true);
    				refreshingList.addRefreshing(mAccount.getUuid(), mFolderName);
    				setPullToRefreshEnabled(false);
    			}
    		}
		...
		if (mCurrentFolder != null && !mSearch.isManualSearch()) {
			...
			if (MailChat.remoteSearchPaging && numResults <= limit) {
			    mController.searchRemoteMessages(mAccount.getUuid(),
			            mCurrentFolder.name,
	                    MailChat.remoteSearchQuery,
	                    null,
	                    null,
	                    mListener);
			} else {
    			List<SearchResult> toProcess = mExtraSearchResults;
    			...
    			mHandler.updateFooter(getString(R.string.pull_to_refresh_end_refreshing_label), true);
    			mController.loadSearchResults(mAccount, mCurrentFolder.name,
    					toProcess, mListener);
			}
		}
