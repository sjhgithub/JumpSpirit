...
import cn.mailchat.activity.MessageInfoHolder;
...
public class MessageProvider extends ContentProvider {
    ...
    protected class MessagesQueryHandler implements QueryHandler {
        ...
        protected MatrixCursor getMessages(final String[] projection) throws InterruptedException {
            ...
            final MessagingController msgController = MessagingController.getInstance(MailChat.app);
            ...
            for (Account account : Preferences.getPreferences(getContext()).getAvailableAccounts()) {
                if (account.getAccountNumber() == accountNumber) {
                    ...
                    try {
                        ...
                        Log.e(MailChat.LOG_TAG, e.getMessage());
                        values[0] = "Unknown";
                        values[1] = 0;
                    }
                }
    ...
    protected static class MonitoredCursor implements CrossProcessCursor {
        ...
        public void close() {
            if (mClosed.compareAndSet(false, true)) {
                mCursor.close();
                Log.d(MailChat.LOG_TAG, "Cursor closed, null'ing & releasing semaphore");
                mCursor = null;
                mSemaphore.release();
            }
        }
    ...
    protected class ThrottlingQueryHandler implements QueryHandler {
        ...
        public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
            ...
            if (!(cursor instanceof CrossProcessCursor)) {
                Log.w(MailChat.LOG_TAG, "Unsupported cursor, returning null: " + cursor);
                mSemaphore.release();
                return null;
            }
            ...
            mScheduledPool.schedule(new Runnable() {
                ...
                public void run() {
                    ...
                    if (monitored != null && !monitored.isClosed()) {
                        Log.w(MailChat.LOG_TAG, "Forcibly closing remotely exposed cursor");
                        try {
                            monitored.close();
                        } catch (Exception e) {
                            Log.w(MailChat.LOG_TAG, "Exception while forcibly closing cursor", e);
                        }
                    }
                }
            }, 30, TimeUnit.SECONDS);
    ...
    protected class MesssageInfoHolderRetrieverListener extends MessagingListener {
        ...
        public void searchStats(AccountStats stats) {
            try {
                ...
                Log.e(MailChat.LOG_TAG, "Unable to return message list back to caller", e);
            }
        }
    }

    public static final String AUTHORITY = "cn.mailchat.messageprovider";
    ...
    public boolean onCreate() {
        ...
        MailChat.registerApplicationAware(new MailChat.ApplicationAware() {
            @Override
            public void initializeComponent(final Application application) {
                Log.v(MailChat.LOG_TAG, "Registering content resolver notifier");
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        if (MailChat.app == null) {
            return 0;
        }

        if (MailChat.DEBUG) {
            Log.v(MailChat.LOG_TAG, "MessageProvider/delete: " + uri);
        }
        ...
        for (Account account : Preferences.getPreferences(getContext()).getAccounts()) {
            if (account.getAccountNumber() == accountId) {
                ...
                if (!account.isAvailable(getContext())) {
                    Log.w(MailChat.LOG_TAG, "not deleting messages because account is unavailable at the moment");
                    return 0;
                }
            }
        }
        ...
        Message msg = null;
        try {
            Folder lf = LocalStore.getLocalInstance(myAccount, MailChat.app).getFolder(folderName);
            int msgCount = lf.getMessageCount();
            if (MailChat.DEBUG) {
                Log.d(MailChat.LOG_TAG, "folder msg count = " + msgCount);
            }
            msg = lf.getMessage(msgUid);
        } catch (MessagingException e) {
            Log.e(MailChat.LOG_TAG, "Unable to retrieve message", e);
        }
        ...
        if ((myAccount != null) && (msg != null)) {
            ...
            controller.deleteMessages(Collections.singletonList(msg),true, null);
        }
    ...
    public String getType(Uri uri) {
        ...
        if (MailChat.DEBUG) {
            Log.v(MailChat.LOG_TAG, "MessageProvider/getType: " + uri);
        }
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        if (MailChat.DEBUG) {
            Log.v(MailChat.LOG_TAG, "MessageProvider/insert: " + uri);
        }
    ...
    public Cursor query(final Uri uri, final String[] projection, final String selection,
        ...
        if (MailChat.DEBUG) {
            Log.v(MailChat.LOG_TAG, "MessageProvider/query: " + uri);
        }
        ...
        try {
            ...
            Log.e(MailChat.LOG_TAG, "Unable to execute query for URI: " + uri, e);
            return null;
        }
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (MailChat.DEBUG) {
            Log.v(MailChat.LOG_TAG, "MessageProvider/update: " + uri);
        }
