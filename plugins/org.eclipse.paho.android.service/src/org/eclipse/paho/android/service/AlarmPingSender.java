...
class AlarmPingSender implements MqttPingSender {
	...
	public AlarmPingSender(MqttService service) {
		...
		that = this;
		prefs = service.getSharedPreferences("mqtt_ping",Context.MODE_PRIVATE);
	}
	...
	class AlarmReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			wakelock.acquire(TIME_OUT);
			Log.w(ClientComms.LOG_COLLECTOR_TAG, "--------------wakelock.acquire()");
			token.setActionCallback(new IMqttActionListener() {
				...
				public void onSuccess(IMqttToken asyncActionToken) {
					...
					savePingSuccessTime(time);
					if(wakelock != null && wakelock.isHeld()){
						wakelock.release();
						Log.w(ClientComms.LOG_COLLECTOR_TAG, "--onSuccess==wakelock.release()---------------");
					}
				}
				...
				public void onFailure(IMqttToken asyncActionToken,
					...
					if(wakelock != null && wakelock.isHeld()){
						wakelock.release();
						Log.w(ClientComms.LOG_COLLECTOR_TAG, "--onFailure==wakelock.release()-------------");
					}
				}

				@Override
				public void onSuccessByTime(IMqttToken asyncActionToken,
						long time) {
	...
	/**
	 * 记录保存每次ping成功的时间
	 * 备注：MessagingController类MQTT回调ConnectSuccess中会更新改值。
	 * @Description:
	 * @see:
	 * @since:
	 * @author: shengli
	 * @date:2015-11-12
	 */
	private void savePingSuccessTime(long time){
		//存储本次PingSuccess时间
		Editor editor =prefs.edit();
		editor.putLong("pingSuccessTime", time);
		editor.commit();
	}
}
