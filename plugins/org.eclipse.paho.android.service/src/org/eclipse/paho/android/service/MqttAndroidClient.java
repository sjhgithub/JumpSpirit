...
public class MqttAndroidClient extends BroadcastReceiver implements
	...
	public void onReceive(Context context, Intent intent) {
		...
		//shengli
		//start
		else if (MqttServiceConstants.SEND_TIME_ACTION.equals(action)) {
			sendTimeAction(data);
		}
		//end
	...
	private void simpleAction(IMqttToken token, Bundle data) {
		if (token != null) {
			...
			if (status == Status.OK) {
				...
				if(timeStamp!=0){
					((MqttTokenAndroid) token).notifyTimeComplete(timeStamp);
				}else{
					//原始只有这个
					((MqttTokenAndroid) token).notifyComplete();
				}
	...
	//shengli
	//start
	private void sendTimeAction(Bundle data) {
		IMqttToken token = getMqttToken(data); // get, don't remove - will
		// remove on delivery
		simpleAction(token, data);
	}
	...
	public IMqttToken getConnectToken() {
		return connectToken;
	}

	public void setConnectToken(IMqttToken connectToken) {
